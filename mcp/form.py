...
def info() -> dict:
    ...
    return {
        ...
        "tier": 7,
        "owner": "Build Team",
        "job": "Form handling, validation, file uploads.",
    }


def run(action: str = "default", **kwargs) -> McpResult:
    """Main entry point for this MCP."""
    write_log("Form MCP", kwargs.get("agent", "Unknown"), action, kwargs)
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("MCP 44: Form MCP")
        print("Owner: Build Team")
        print("Tier: 7 — Specialized")
        print()
        print("Usage: python form.py <command> [args]")
