...
def info() -> dict:
    ...
    return {
        ...
        "tier": 2,
        "owner": "Hermes",
        "job": "Track what's built, enable crash recovery. Hermes reads this to resume after a crash.",
    }


def run(action: str = "default", **kwargs) -> McpResult:
    """Main entry point for this MCP."""
    write_log("Progress MCP", kwargs.get("agent", "Unknown"), action, kwargs)
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("MCP 6: Progress MCP")
        print("Owner: Hermes")
        print("Tier: 2 — Core Ops")
        print()
        print("Usage: python progress.py <command> [args]")
