...
def info() -> dict:
    ...
    return {
        ...
        "tier": 7,
        "owner": "Build Team",
        "job": "Paystack, Stripe, refunds, webhooks.",
    }


def run(action: str = "default", **kwargs) -> McpResult:
    """Main entry point for this MCP."""
    write_log("Payment MCP", kwargs.get("agent", "Unknown"), action, kwargs)
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("MCP 43: Payment MCP")
        print("Owner: Build Team")
        print("Tier: 7 — Specialized")
        print()
        print("Usage: python payment.py <command> [args]")
