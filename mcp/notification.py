...
def info() -> dict:
    ...
    return {
        ...
        "tier": 6,
        "owner": "Build Team",
        "job": "Send emails (Resend), SMS (Termii), push notifications.",
    }


def run(action: str = "default", **kwargs) -> McpResult:
    """Main entry point for this MCP."""
    write_log("Notification MCP", kwargs.get("agent", "Unknown"), action, kwargs)
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("MCP 37: Notification MCP")
        print("Owner: Build Team")
        print("Tier: 6 — Runtime")
        print()
        print("Usage: python notification.py <command> [args]")
