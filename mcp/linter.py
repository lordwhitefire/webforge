...
def info() -> dict:
    ...
    return {
        ...
        "tier": 2,
        "owner": "Build Team",
        "job": "Run ESLint, Prettier, type-check. Fails the build if dirty.",
    }


def run(action: str = "default", **kwargs) -> McpResult:
    """Main entry point for this MCP."""
    write_log("Linter MCP", kwargs.get("agent", "Unknown"), action, kwargs)
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("MCP 12: Linter MCP")
        print("Owner: Build Team")
        print("Tier: 2 — Core Ops")
        print()
        print("Usage: python linter.py <command> [args]")
