...
def info() -> dict:
    ...
    return {
        "id": "m10",
        "name": "CEO Communication MCP",
        "tier": 2,
        "owner": "CEO",
        "job": "Bridge between developer and the pipeline. Routes questions and answers.",
    }


def run(action: str = "default", **kwargs) -> McpResult:
    """Main entry point for this MCP."""
    write_log("CEO Communication MCP", kwargs.get("agent", "Unknown"), action, kwargs)
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("MCP 10: CEO Communication MCP")
        print("Owner: CEO")
        print("Tier: 2 — Core Ops")
        print()
        print("Usage: python ceo_communication.py <command> [args]")
