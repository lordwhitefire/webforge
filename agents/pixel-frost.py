...
def do_work(task):
    ...
    try:
        ...
        log_to_memory(f"PIXEL-PixelFrost ran tests for areas 76-80")
        return f"Tests run for areas 76-80."
    except:
        return f"Test run failed for areas 76-80."
...
def run(message="work", context=None):
    ...
    if not my_tasks:
        return {
            "agent": "PixelFrost",
            "action": "idle",
            "message": f"I am PixelFrost. No tasks assigned to me on the board. I own areas 76-80.",
            "next_step": None,
        }
    ...
    # Log to memory
    log_to_memory(f"PixelFrost COMPLETED {task_id}: {task_title} — {result_message}")
    ...
    return {
        ...
        "message": f"I am PixelFrost. I worked on {task_id}: {task_title}.\n  Result: {result_message}\n  Task marked DONE.",
        "next_step": None,
    }
