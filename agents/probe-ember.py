#!/usr/bin/env python3
"""
ProbeEmber — Intelligence Department
STANDALONE script. Checks board for assigned tasks and works on them.

Role: I am Probe-Ember. I am a Probe Agent in the Intelligence team. I report to Probe-Lead.
Areas: 76-80
"""
...
def do_work(task):
    """Probe agent: scan project for areas 76-80."""
    ...
    try:
        ...
        log_to_memory(f"PROBE-ProbeEmber scanned: {files} files for areas 76-80")
        return f"Probed areas 76-80. Found {files} files."
...
def run(message="work", context=None):
    ...
    if not my_tasks:
        return {
            "agent": "ProbeEmber",
            "action": "idle",
            "message": f"I am ProbeEmber. No tasks assigned to me on the board. I own areas 76-80.",
            "next_step": None,
        }
    ...
    # Log to memory
    log_to_memory(f"ProbeEmber COMPLETED {task_id}: {task_title} — {result_message}")
    ...
    return {
        ...
        "message": f"I am ProbeEmber. I worked on {task_id}: {task_title}.\n  Result: {result_message}\n  Task marked DONE.",
        "next_step": None,
    }
