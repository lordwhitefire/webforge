...
def do_work(task):
    ...
    log_to_memory(f"JANUS-JanusHeron security check for areas 71-75")
    return f"Security check complete for areas 71-75."
...
def run(message="work", context=None):
    ...
    if not my_tasks:
        return {
            "agent": "JanusHeron",
            "action": "idle",
            "message": f"I am JanusHeron. No tasks assigned to me on the board. I own areas 71-75.",
            "next_step": None,
        }
    ...
    # Log to memory
    log_to_memory(f"JanusHeron COMPLETED {task_id}: {task_title} — {result_message}")
    ...
    return {
        ...
        "message": f"I am JanusHeron. I worked on {task_id}: {task_title}.\n  Result: {result_message}\n  Task marked DONE.",
        "next_step": None,
    }
