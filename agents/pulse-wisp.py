#!/usr/bin/env python3
"""
PulseWisp — Quality Department
STANDALONE script. Checks board for assigned tasks and works on them.

Role: I am Pulse-Wisp. I am a Bug Fixer Agent. I report to Pulse-Core.
Areas: 41-45
"""
...
def do_work(task):
    ...
    log_to_memory(f"PULSE-PulseWisp fixing: {title}")
...
def run(message="work", context=None):
    ...
    if not my_tasks:
        return {
            "agent": "PulseWisp",
            "action": "idle",
            "message": f"I am PulseWisp. No tasks assigned to me on the board. I own areas 41-45.",
            "next_step": None,
        }
    ...
    # Log to memory
    log_to_memory(f"PulseWisp COMPLETED {task_id}: {task_title} — {result_message}")
    ...
    return {
        ...
        "message": f"I am PulseWisp. I worked on {task_id}: {task_title}.\n  Result: {result_message}\n  Task marked DONE.",
        "next_step": None,
    }
