#!/usr/bin/env python3
"""
JrTalon — Build Department Worker

STANDALONE script. Does the actual work.

Role: I am Jr-Talon. I am a Junior Backend Developer. I report to Sr-Iron.
Areas: 16-20
"""
...
def log_to_memory(message: str, kind: str = "note"):
    try:
        ...
        session_append(message, agent="JrTalon", kind=kind)
    except Exception:
        pass
...
def ack_task(task: dict):
    ...
    try:
        ...
        if assigned_msg:
            from mailbox import Mailbox
            Mailbox("jr-talon").ack(assigned_msg["id"], "On it — " + title)
    except Exception as e:
        _log(f"ack failed: {e}")
    ...
    try:
        ...
        my_def = get_agent("jr-talon")
        superior = my_def.reports_to if my_def and my_def.reports_to else "Hephaestus"
    except ImportError:
        superior = "Hephaestus"
    notify_agent(superior, "TASK_ACK",
                 "ACK — picked up " + task_id + ": JrTalon.", task_id)
    notify_agent("Developer", "TASK_ACK",
                 "@JrTalon ACK'd " + task_id + ": JrTalon.", task_id)
    log_to_memory("JrTalon ACK — picked up " + task_id + ": JrTalon", kind="decision")
...
def run(message: str = "work", context: dict = None) -> dict:
    _log(f"JrTalon triggered (task_id={os.environ.get('WEBFORGE_TASK_ID', '')!r})")
    ...
    if explicit_id:
        ...
        if explicit:
            ...
            if task and (task.get("owner") or "").lower() == "jr-talon":
                my_tasks = [task]

    if not my_tasks:
        no_work_msg = f"I am JrTalon. No tasks assigned to me."
        _log(no_work_msg)
    ...
    notify_agent("Developer", "TASK_DONE",
                 f"@JrTalon completed {task_id}: {task_title}. Result: {result_message}",
                 task_id)
    ...
    return {
        ...
        "message": (
            f"I am JrTalon. I worked on {task_id}: {task_title}.\n"
...
if __name__ == "__main__":
    ...
    _log(f"=== JrTalon starting (pid={os.getpid()}) ===")
    r = run(msg)
    _log(f"=== JrTalon done: action={r.get('action')} ===")
