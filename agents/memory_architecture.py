#!/usr/bin/env python3
"""
MemoryArchitecture — Documentation Department
STANDALONE script. Checks board for assigned tasks and works on them.

Role: I am Memory-Architecture. I am part of the Memory Team. I report to Quill.
Areas: N/A
"""
...
def log_to_memory(message):
    ...
    try:
        subprocess.run(["python3", str(MCP_DIR / "memory.py"), "session-append",
                       message, "MemoryArchitecture", "note"],
...
def run(message="work", context=None):
    ...
    if not my_tasks:
        return {
            ...
            "action": "idle",
            "message": f"I am MemoryArchitecture. No tasks assigned to me on the board. I own areas assigned.",
            "next_step": None,
        }
    ...
    # Log to memory
    log_to_memory(f"MemoryArchitecture COMPLETED {task_id}: {task_title} — {result_message}")
    ...
    return {
        ...
        "message": f"I am MemoryArchitecture. I worked on {task_id}: {task_title}.\n  Result: {result_message}\n  Task marked DONE.",
        "next_step": None,
    }
