...
def do_work(task):
    ...
    log_to_memory(f"JANUS-JanusSlate security check for areas 26-30")
    return f"Security check complete for areas 26-30."
...
def run(message="work", context=None):
    ...
    if not my_tasks:
        return {
            "agent": "JanusSlate",
            "action": "idle",
            "message": f"I am JanusSlate. No tasks assigned to me on the board. I own areas 26-30.",
            "next_step": None,
        }
    ...
    # Log to memory
    log_to_memory(f"JanusSlate COMPLETED {task_id}: {task_title} — {result_message}")
    ...
    return {
        ...
        "message": f"I am JanusSlate. I worked on {task_id}: {task_title}.\n  Result: {result_message}\n  Task marked DONE.",
        "next_step": None,
    }
