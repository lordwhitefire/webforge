...
def do_work(task):
    ...
    try:
        ...
        log_to_memory(f"PIXEL-PixelSage ran tests for areas 01-05")
        return f"Tests run for areas 01-05."
    except:
        return f"Test run failed for areas 01-05."
...
def run(message="work", context=None):
    ...
    if not my_tasks:
        return {
            "agent": "PixelSage",
            "action": "idle",
            "message": f"I am PixelSage. No tasks assigned to me on the board. I own areas 01-05.",
            "next_step": None,
        }
    ...
    # Log to memory
    log_to_memory(f"PixelSage COMPLETED {task_id}: {task_title} — {result_message}")
    ...
    return {
        ...
        "message": f"I am PixelSage. I worked on {task_id}: {task_title}.\n  Result: {result_message}\n  Task marked DONE.",
        "next_step": None,
    }
