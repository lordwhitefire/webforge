#!/usr/bin/env python3
"""
JrHawk — Build Department Worker

STANDALONE script. Does the actual work.

Role: I am JrHawk. I am a Junior Frontend Developer. I report to Sr-Quill2.
Areas: 01-05
"""
...
def log_to_memory(message: str, kind: str = "note"):
    try:
        ...
        session_append(message, agent="JrHawk", kind=kind)
    except Exception:
        pass
...
def ack_task(task: dict):
    ...
    try:
        ...
        if assigned_msg:
            from mailbox import Mailbox
            Mailbox("jr-hawk").ack(assigned_msg["id"], "On it — " + title)
    except Exception as e:
        _log(f"ack failed: {e}")
    ...
    notify_agent("Developer", "TASK_ACK",
                 "@JrHawk ACK'd " + task_id + ": Junior Frontend Developer.", task_id)
    log_to_memory("JrHawk ACK — picked up " + task_id + ": Junior Frontend Developer", kind="decision")
...
def run(message: str = "work", context: dict = None) -> dict:
    _log(f"JrHawk triggered (task_id={os.environ.get('WEBFORGE_TASK_ID', '')!r})")
    ...
    if explicit_id:
        ...
        if explicit:
            ...
            if task and (task.get("owner") or "").lower() == "jr-hawk":
                my_tasks = [task]

    if not my_tasks:
        no_work_msg = f"I am JrHawk. No tasks assigned to me."
        _log(no_work_msg)
    ...
    notify_agent("Developer", "TASK_DONE",
                 f"@JrHawk completed {task_id}: {task_title}. Result: {result_message}",
                 task_id)
    log_to_memory(f"JrHawk COMPLETED {task_id}: {task_title} — {result_message}",
                  kind="decision")
    ...
    return {
        ...
        "message": (
            f"I am JrHawk. I worked on {task_id}: {task_title}.\n"
...
if __name__ == "__main__":
    ...
    _log(f"=== JrHawk starting (pid={os.getpid()}) ===")
    r = run(msg)
    _log(f"=== JrHawk done: action={r.get('action')} ===")
