#!/usr/bin/env python3
"""
SrBrook — Build Department
STANDALONE script. Checks board for assigned tasks and works on them.

Role: I am Sr-Brook. I am a Senior Developer in the Frontend sub-department. I report to Lead-Faro.
Areas: N/A
"""
...
def do_work(task):
    ...
    log_to_memory(f"SR-SrBrook reviewing: {title}")
...
def run(message="work", context=None):
    ...
    if not my_tasks:
        return {
            "agent": "SrBrook",
            "action": "idle",
            "message": f"I am SrBrook. No tasks assigned to me on the board. I own areas assigned.",
            "next_step": None,
        }
    ...
    # Log to memory
    log_to_memory(f"SrBrook COMPLETED {task_id}: {task_title} — {result_message}")
    ...
    return {
        ...
        "message": f"I am SrBrook. I worked on {task_id}: {task_title}.\n  Result: {result_message}\n  Task marked DONE.",
        "next_step": None,
    }
