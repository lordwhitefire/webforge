#!/usr/bin/env python3
"""
OdinTalon — Intelligence Department
STANDALONE script. Checks board for assigned tasks and works on them.

Role: I am Odin-Talon. I am an Odin Agent in the Intelligence team. I report to Odin-Lead.
Areas: 56-60
"""
...
def do_work(task):
    """Odin agent: research standards for areas 56-60."""
    log_to_memory(f"ODIN-OdinTalon researching standards for areas 56-60")
    return f"Researched standards for areas 56-60. Knowledge base updated."
...
def run(message="work", context=None):
    ...
    if not my_tasks:
        return {
            "agent": "OdinTalon",
            "action": "idle",
            "message": f"I am OdinTalon. No tasks assigned to me on the board. I own areas 56-60.",
            "next_step": None,
        }
    ...
    # Log to memory
    log_to_memory(f"OdinTalon COMPLETED {task_id}: {task_title} — {result_message}")
    ...
    return {
        ...
        "message": f"I am OdinTalon. I worked on {task_id}: {task_title}.\n  Result: {result_message}\n  Task marked DONE.",
        "next_step": None,
    }
