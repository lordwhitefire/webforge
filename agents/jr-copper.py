#!/usr/bin/env python3
"""
JrCopper — Build Department Worker

STANDALONE script. Does the actual work.

Role: I am JrCopper. I am a Junior Backend Developer. I report to Sr-Iron.
Areas: 21-25
"""
...
def log_to_memory(message: str, kind: str = "note"):
    try:
        ...
        session_append(message, agent="JrCopper", kind=kind)
    except Exception:
        pass
...
def ack_task(task: dict):
    ...
    try:
        ...
        if assigned_msg:
            from mailbox import Mailbox
            Mailbox("jr-copper").ack(assigned_msg["id"], "On it — " + title)
    except Exception as e:
        _log(f"ack failed: {e}")
    ...
    notify_agent("Developer", "TASK_ACK",
                 "@JrCopper ACK'd " + task_id + ": Junior Backend Developer.", task_id)
    log_to_memory("JrCopper ACK — picked up " + task_id + ": Junior Backend Developer", kind="decision")
...
def run(message: str = "work", context: dict = None) -> dict:
    _log(f"JrCopper triggered (task_id={os.environ.get('WEBFORGE_TASK_ID', '')!r})")
    ...
    if explicit_id:
        ...
        if explicit:
            ...
            if task and (task.get("owner") or "").lower() == "jr-copper":
                my_tasks = [task]

    if not my_tasks:
        no_work_msg = f"I am JrCopper. No tasks assigned to me."
        _log(no_work_msg)
    ...
    notify_agent("Developer", "TASK_DONE",
                 f"@JrCopper completed {task_id}: {task_title}. Result: {result_message}",
                 task_id)
    log_to_memory(f"JrCopper COMPLETED {task_id}: {task_title} — {result_message}",
                  kind="decision")
    ...
    return {
        ...
        "message": (
            f"I am JrCopper. I worked on {task_id}: {task_title}.\n"
...
if __name__ == "__main__":
    ...
    _log(f"=== JrCopper starting (pid={os.getpid()}) ===")
    r = run(msg)
    _log(f"=== JrCopper done: action={r.get('action')} ===")
