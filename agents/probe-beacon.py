...
Role: I am Probe-Beacon. I am a Probe Agent in the Intelligence team. I report to Probe-Lead.
Areas: 11-15
"""
...
def do_work(task):
    """Probe agent: scan project for areas 11-15."""
    ...
    try:
        ...
        log_to_memory(f"PROBE-ProbeBeacon scanned: {files} files for areas 11-15")
        return f"Probed areas 11-15. Found {files} files."
...
def run(message="work", context=None):
    ...
    if not my_tasks:
        return {
            "agent": "ProbeBeacon",
            "action": "idle",
            "message": f"I am ProbeBeacon. No tasks assigned to me on the board. I own areas 11-15.",
            "next_step": None,
        }
    ...
    # Log to memory
    log_to_memory(f"ProbeBeacon COMPLETED {task_id}: {task_title} — {result_message}")
    ...
    return {
        ...
        "message": f"I am ProbeBeacon. I worked on {task_id}: {task_title}.\n  Result: {result_message}\n  Task marked DONE.",
        "next_step": None,
    }
