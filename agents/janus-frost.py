...
def do_work(task):
    ...
    log_to_memory(f"JANUS-JanusFrost security check for areas 76-80")
    return f"Security check complete for areas 76-80."
...
def run(message="work", context=None):
    ...
    if not my_tasks:
        return {
            "agent": "JanusFrost",
            "action": "idle",
            "message": f"I am JanusFrost. No tasks assigned to me on the board. I own areas 76-80.",
            "next_step": None,
        }
    ...
    # Log to memory
    log_to_memory(f"JanusFrost COMPLETED {task_id}: {task_title} — {result_message}")
    ...
    return {
        ...
        "message": f"I am JanusFrost. I worked on {task_id}: {task_title}.\n  Result: {result_message}\n  Task marked DONE.",
        "next_step": None,
    }
