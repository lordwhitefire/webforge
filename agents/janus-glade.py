...
def do_work(task):
    ...
    log_to_memory(f"JANUS-JanusGlade security check for areas 66-70")
    return f"Security check complete for areas 66-70."
...
def run(message="work", context=None):
    ...
    if not my_tasks:
        return {
            "agent": "JanusGlade",
            "action": "idle",
            "message": f"I am JanusGlade. No tasks assigned to me on the board. I own areas 66-70.",
            "next_step": None,
        }
    ...
    # Log to memory
    log_to_memory(f"JanusGlade COMPLETED {task_id}: {task_title} — {result_message}")
    ...
    return {
        ...
        "message": f"I am JanusGlade. I worked on {task_id}: {task_title}.\n  Result: {result_message}\n  Task marked DONE.",
        "next_step": None,
    }
