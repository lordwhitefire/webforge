#!/usr/bin/env python3
"""
ProbeCrag — Intelligence Department
STANDALONE script. Checks board for assigned tasks and works on them.

Role: I am Probe-Crag. I am a Probe Agent in the Intelligence team. I report to Probe-Lead.
Areas: 66-70
"""
...
def do_work(task):
    """Probe agent: scan project for areas 66-70."""
    ...
    try:
        ...
        log_to_memory(f"PROBE-ProbeCrag scanned: {files} files for areas 66-70")
        return f"Probed areas 66-70. Found {files} files."
...
def run(message="work", context=None):
    ...
    if not my_tasks:
        return {
            "agent": "ProbeCrag",
            "action": "idle",
            "message": f"I am ProbeCrag. No tasks assigned to me on the board. I own areas 66-70.",
            "next_step": None,
        }
    ...
    # Log to memory
    log_to_memory(f"ProbeCrag COMPLETED {task_id}: {task_title} — {result_message}")
    ...
    return {
        ...
        "message": f"I am ProbeCrag. I worked on {task_id}: {task_title}.\n  Result: {result_message}\n  Task marked DONE.",
        "next_step": None,
    }
