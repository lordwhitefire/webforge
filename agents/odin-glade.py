#!/usr/bin/env python3
"""
OdinGlade — Intelligence Department
STANDALONE script. Checks board for assigned tasks and works on them.

Role: I am Odin-Glade. I am an Odin Agent in the Intelligence team. I report to Odin-Lead.
Areas: 66-70
"""
...
def do_work(task):
    """Odin agent: research standards for areas 66-70."""
    log_to_memory(f"ODIN-OdinGlade researching standards for areas 66-70")
    return f"Researched standards for areas 66-70. Knowledge base updated."
...
def run(message="work", context=None):
    ...
    if not my_tasks:
        return {
            "agent": "OdinGlade",
            "action": "idle",
            "message": f"I am OdinGlade. No tasks assigned to me on the board. I own areas 66-70.",
            "next_step": None,
        }
    ...
    # Log to memory
    log_to_memory(f"OdinGlade COMPLETED {task_id}: {task_title} — {result_message}")
    ...
    return {
        ...
        "message": f"I am OdinGlade. I worked on {task_id}: {task_title}.\n  Result: {result_message}\n  Task marked DONE.",
        "next_step": None,
    }
