#!/usr/bin/env python3
"""
JrChromium — Build Department Worker

STANDALONE script. Does the actual work.

Role: I am JrChromium. I am a Junior Backend Developer. I report to Sr-Cloud.
Areas: 71-75
"""
...
def log_to_memory(message: str, kind: str = "note"):
    try:
        ...
        session_append(message, agent="JrChromium", kind=kind)
    except Exception:
        pass
...
def ack_task(task: dict):
    ...
    try:
        ...
        if assigned_msg:
            from mailbox import Mailbox
            Mailbox("jr-chromium").ack(assigned_msg["id"], "On it — " + title)
    except Exception as e:
        _log(f"ack failed: {e}")
...
def run(message: str = "work", context: dict = None) -> dict:
    _log(f"JrChromium triggered (task_id={os.environ.get('WEBFORGE_TASK_ID', '')!r})")
    ...
    if explicit_id:
        ...
        if explicit:
            ...
            if task and (task.get("owner") or "").lower() == "jr-chromium":
                my_tasks = [task]

    if not my_tasks:
        no_work_msg = f"I am JrChromium. No tasks assigned to me."
        _log(no_work_msg)
    ...
    try:
        notify_agent("Developer", "TASK_PROGRESS",
                     f"@JrChromium → {task_id}: Starting work", task_id)
        checkpoint("working", {})
        result_message = do_work(task)
        notify_agent("Developer", "TASK_PROGRESS",
                     f"@JrChromium → {task_id}: {result_message}", task_id)
        checkpoint("work_complete", {"result": result_message})
    except Exception as e:
        err_msg = f"Work failed: {e}"
        _log(err_msg)
        notify_agent("Developer", "TASK_BLOCKED",
                     f"@JrChromium failed on {task_id}: {e}", task_id, priority=2)
        try:
        ...
        return {
            "agent": "JrChromium", "action": "failed",
    ...
    notify_agent("Developer", "TASK_DONE",
                 ...
                 task_id)
    log_to_memory(f"JrChromium COMPLETED {task_id}: {task_title} — {result_message}",
                  kind="decision")
    ...
    return {
        ...
        "message": (
            f"I am JrChromium. I worked on {task_id}: {task_title}.\n"
...
if __name__ == "__main__":
    ...
    _log(f"=== JrChromium starting (pid={os.getpid()}) ===")
    r = run(msg)
    _log(f"=== JrChromium done: action={r.get('action')} ===")
