...
def do_work(task):
    ...
    log_to_memory(f"JANUS-JanusReed security check for areas 06-10")
    return f"Security check complete for areas 06-10."
...
def run(message="work", context=None):
    ...
    if not my_tasks:
        return {
            "agent": "JanusReed",
            "action": "idle",
            "message": f"I am JanusReed. No tasks assigned to me on the board. I own areas 06-10.",
            "next_step": None,
        }
    ...
    # Log to memory
    log_to_memory(f"JanusReed COMPLETED {task_id}: {task_title} — {result_message}")
    ...
    return {
        ...
        "message": f"I am JanusReed. I worked on {task_id}: {task_title}.\n  Result: {result_message}\n  Task marked DONE.",
        "next_step": None,
    }
