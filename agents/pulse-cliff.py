#!/usr/bin/env python3
"""
PulseCliff — Quality Department
STANDALONE script. Checks board for assigned tasks and works on them.

Role: I am Pulse-Cliff. I am a Bug Fixer Agent. I report to Pulse-Core.
Areas: 16-20
"""
...
def do_work(task):
    ...
    log_to_memory(f"PULSE-PulseCliff fixing: {title}")
...
def run(message="work", context=None):
    ...
    if not my_tasks:
        return {
            "agent": "PulseCliff",
            "action": "idle",
            "message": f"I am PulseCliff. No tasks assigned to me on the board. I own areas 16-20.",
            "next_step": None,
        }
    ...
    # Log to memory
    log_to_memory(f"PulseCliff COMPLETED {task_id}: {task_title} — {result_message}")
    ...
    return {
        ...
        "message": f"I am PulseCliff. I worked on {task_id}: {task_title}.\n  Result: {result_message}\n  Task marked DONE.",
        "next_step": None,
    }
