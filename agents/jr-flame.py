#!/usr/bin/env python3
"""
JrFlame — Build Department Worker

STANDALONE script. Does the actual work.

Role: I am JrFlame. I am a Junior Database/Infra Developer. I report to Sr-Steel.
Areas: 41-45
"""
...
def log_to_memory(message: str, kind: str = "note"):
    try:
        ...
        session_append(message, agent="JrFlame", kind=kind)
    except Exception:
        pass
...
def ack_task(task: dict):
    ...
    try:
        ...
        if assigned_msg:
            from mailbox import Mailbox
            Mailbox("jr-flame").ack(assigned_msg["id"], "On it — " + title)
    except Exception as e:
        _log(f"ack failed: {e}")
    ...
    notify_agent("Developer", "TASK_ACK",
                 "@JrFlame ACK'd " + task_id + ": Junior Database/Infra Developer.", task_id)
    log_to_memory("JrFlame ACK — picked up " + task_id + ": Junior Database/Infra Developer", kind="decision")
...
def run(message: str = "work", context: dict = None) -> dict:
    _log(f"JrFlame triggered (task_id={os.environ.get('WEBFORGE_TASK_ID', '')!r})")
    ...
    if explicit_id:
        ...
        if explicit:
            ...
            if task and (task.get("owner") or "").lower() == "jr-flame":
                my_tasks = [task]

    if not my_tasks:
        no_work_msg = f"I am JrFlame. No tasks assigned to me."
        _log(no_work_msg)
    ...
    notify_agent("Developer", "TASK_DONE",
                 f"@JrFlame completed {task_id}: {task_title}. Result: {result_message}",
                 task_id)
    log_to_memory(f"JrFlame COMPLETED {task_id}: {task_title} — {result_message}",
                  kind="decision")
    ...
    return {
        ...
        "message": (
            f"I am JrFlame. I worked on {task_id}: {task_title}.\n"
...
if __name__ == "__main__":
    ...
    _log(f"=== JrFlame starting (pid={os.getpid()}) ===")
    r = run(msg)
    _log(f"=== JrFlame done: action={r.get('action')} ===")
