#!/usr/bin/env python3
"""
PulseSage — Quality Department
STANDALONE script. Checks board for assigned tasks and works on them.

Role: I am Pulse-Sage. I am a Bug Fixer Agent. I report to Pulse-Core.
Areas: 01-05
"""
...
def do_work(task):
    ...
    log_to_memory(f"PULSE-PulseSage fixing: {title}")
...
def run(message="work", context=None):
    ...
    if not my_tasks:
        return {
            "agent": "PulseSage",
            "action": "idle",
            "message": f"I am PulseSage. No tasks assigned to me on the board. I own areas 01-05.",
            "next_step": None,
        }
    ...
    # Log to memory
    log_to_memory(f"PulseSage COMPLETED {task_id}: {task_title} — {result_message}")
    ...
    return {
        ...
        "message": f"I am PulseSage. I worked on {task_id}: {task_title}.\n  Result: {result_message}\n  Task marked DONE.",
        "next_step": None,
    }
