#!/usr/bin/env python3
"""
JrBronze — Build Department Worker

STANDALONE script. Does the actual work.

Role: I am JrBronze. I am a Junior Backend Developer. I report to Sr-Iron.
Areas: 26-30
"""
...
def log_to_memory(message: str, kind: str = "note"):
    try:
        ...
        session_append(message, agent="JrBronze", kind=kind)
    except Exception:
        pass
...
def ack_task(task: dict):
    ...
    try:
        ...
        if assigned_msg:
            from mailbox import Mailbox
            Mailbox("jr-bronze").ack(assigned_msg["id"], "On it — " + title)
    except Exception as e:
        _log(f"ack failed: {e}")
    ...
    notify_agent("Developer", "TASK_ACK",
                 "@JrBronze ACK'd " + task_id + ": Junior Backend Developer.", task_id)
    log_to_memory("JrBronze ACK — picked up " + task_id + ": Junior Backend Developer", kind="decision")
...
def run(message: str = "work", context: dict = None) -> dict:
    _log(f"JrBronze triggered (task_id={os.environ.get('WEBFORGE_TASK_ID', '')!r})")
    ...
    if explicit_id:
        ...
        if explicit:
            ...
            if task and (task.get("owner") or "").lower() == "jr-bronze":
                my_tasks = [task]

    if not my_tasks:
        no_work_msg = f"I am JrBronze. No tasks assigned to me."
        _log(no_work_msg)
    ...
    notify_agent("Developer", "TASK_DONE",
                 f"@JrBronze completed {task_id}: {task_title}. Result: {result_message}",
                 task_id)
    log_to_memory(f"JrBronze COMPLETED {task_id}: {task_title} — {result_message}",
                  kind="decision")
    ...
    return {
        ...
        "message": (
            f"I am JrBronze. I worked on {task_id}: {task_title}.\n"
...
if __name__ == "__main__":
    ...
    _log(f"=== JrBronze starting (pid={os.getpid()}) ===")
    r = run(msg)
    _log(f"=== JrBronze done: action={r.get('action')} ===")
