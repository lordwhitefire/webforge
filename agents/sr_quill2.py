#!/usr/bin/env python3
"""
SrQuill2 — Build Department
STANDALONE script. Checks board for assigned tasks and works on them.

Role: I am Sr-Quill2. I am a Senior Developer in the Frontend sub-department. I report to Lead-Faro.
Areas: N/A
"""
...
def do_work(task):
    ...
    log_to_memory(f"SR-SrQuill2 reviewing: {title}")
...
def run(message="work", context=None):
    ...
    if not my_tasks:
        return {
            "agent": "SrQuill2",
            "action": "idle",
            "message": f"I am SrQuill2. No tasks assigned to me on the board. I own areas assigned.",
            "next_step": None,
        }
    ...
    # Log to memory
    log_to_memory(f"SrQuill2 COMPLETED {task_id}: {task_title} — {result_message}")
    ...
    return {
        ...
        "message": f"I am SrQuill2. I worked on {task_id}: {task_title}.\n  Result: {result_message}\n  Task marked DONE.",
        "next_step": None,
    }
