...
def do_work(task):
    """Probe agent: scan project for areas 51-55."""
    ...
    try:
        ...
        log_to_memory(f"PROBE-ProbeThorne scanned: {files} files for areas 51-55")
        return f"Probed areas 51-55. Found {files} files."
...
def run(message="work", context=None):
    ...
    if not my_tasks:
        return {
            "agent": "ProbeThorne",
            "action": "idle",
            "message": f"I am ProbeThorne. No tasks assigned to me on the board. I own areas 51-55.",
            "next_step": None,
        }
    ...
    # Log to memory
    log_to_memory(f"ProbeThorne COMPLETED {task_id}: {task_title} — {result_message}")
    ...
    return {
        ...
        "message": f"I am ProbeThorne. I worked on {task_id}: {task_title}.\n  Result: {result_message}\n  Task marked DONE.",
        "next_step": None,
    }
