...
Areas: 51-55
Skill file: skills/intelligence/probe-thorne.md
"""
...
def run(message, context=None):
    """Probe agent: scans project for areas 51-55."""
    ...
    try:
        subprocess.run(["python3", str(MCP_DIR / "memory.py"), "session-append",
                       f"PROBE-ProbeThorne scanned project. Files: {scan_data.get('stats', {}).get('total_files', '?')}",
                       "ProbeThorne", "note"],
    ...
    return {
        ...
        "action": "probe",
        "areas": "51-55",
        "message": f"I am ProbeThorne. I probed areas 51-55. Found {scan_data.get('stats', {}).get('total_files', '?')} files. Findings written to memory.",
        "scan": scan_data,
        "next_step": None,
    }
