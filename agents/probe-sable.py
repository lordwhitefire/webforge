...
Role: I am Probe-Sable. I am a Probe Agent in the Intelligence team. I report to Probe-Lead.
Areas: 16-20
"""
...
def do_work(task):
    """Probe agent: scan project for areas 16-20."""
    ...
    try:
        ...
        log_to_memory(f"PROBE-ProbeSable scanned: {files} files for areas 16-20")
        return f"Probed areas 16-20. Found {files} files."
...
def run(message="work", context=None):
    ...
    if not my_tasks:
        return {
            "agent": "ProbeSable",
            "action": "idle",
            "message": f"I am ProbeSable. No tasks assigned to me on the board. I own areas 16-20.",
            "next_step": None,
        }
    ...
    # Log to memory
    log_to_memory(f"ProbeSable COMPLETED {task_id}: {task_title} — {result_message}")
    ...
    return {
        ...
        "message": f"I am ProbeSable. I worked on {task_id}: {task_title}.\n  Result: {result_message}\n  Task marked DONE.",
        "next_step": None,
    }
