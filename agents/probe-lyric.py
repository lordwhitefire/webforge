#!/usr/bin/env python3
"""
ProbeLyric — Intelligence Department
STANDALONE script. Checks board for assigned tasks and works on them.

Role: I am Probe-Lyric. I am a Probe Agent in the Intelligence team. I report to Probe-Lead.
Areas: 81-82
"""
...
def do_work(task):
    """Probe agent: scan project for areas 81-82."""
    ...
    try:
        ...
        log_to_memory(f"PROBE-ProbeLyric scanned: {files} files for areas 81-82")
        return f"Probed areas 81-82. Found {files} files."
...
def run(message="work", context=None):
    ...
    if not my_tasks:
        return {
            "agent": "ProbeLyric",
            "action": "idle",
            "message": f"I am ProbeLyric. No tasks assigned to me on the board. I own areas 81-82.",
            "next_step": None,
        }
    ...
    # Log to memory
    log_to_memory(f"ProbeLyric COMPLETED {task_id}: {task_title} — {result_message}")
    ...
    return {
        ...
        "message": f"I am ProbeLyric. I worked on {task_id}: {task_title}.\n  Result: {result_message}\n  Task marked DONE.",
        "next_step": None,
    }
