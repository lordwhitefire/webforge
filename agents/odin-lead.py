#!/usr/bin/env python3
"""
Odin-Lead — Intelligence Department
Team Lead for the Odin Team. Reports to Athena. Manages all Odin-* agents.

Role: I am Odin-Lead. I am the Odin Team Lead in the Intelligence department. I report to Athena.
Areas: N/A
"""
...
CONSTRAINTS = {
    "role": "Odin Team Lead — manages Probe agents, reports to Athena",
...
def log_to_memory(message, kind="note"):
    try:
        ...
        session_append(message, agent="Odin-Lead", kind=kind)
    except Exception:
        pass
...
def run(message="work", context=None):
    _log(f"Odin-Lead triggered (task_id={os.environ.get('WEBFORGE_TASK_ID', '')!r})")

    my_tasks = check_my_tasks()
    _log(f"Found {len(my_tasks)} task(s)")

    if not my_tasks:
        return {"agent": "Odin-Lead", "action": "idle",
                "message": "I am Odin-Lead. No tasks assigned to me.", "next_step": None}

    # As a lead, delegate to odin-* workers
    task = my_tasks[0]
    task_id = task["id"]
    ...
    probe_workers = [f"odin-{n}" for n in [
    ...
    try:
        from mailbox import Mailbox
        Mailbox("Odin-Lead").send(
            to=worker, msg_type="TASK_ASSIGNED",
            subject=f"Delegated: {title[:60]}",
            body=f"@Odin-Lead delegated task {task_id} to you.",
    ...
    return {
        "agent": "Odin-Lead", "action": "delegated",
        "task_id": task_id, "delegated_to": worker,
        "message": f"I am Odin-Lead. Delegated {task_id} to @{worker}.",
        "next_step": None,
    }
