#!/usr/bin/env python3
"""
OdinSlate — Intelligence Department
STANDALONE script. Checks board for assigned tasks and works on them.

Role: I am Odin-Slate. I am an Odin Agent in the Intelligence team. I report to Odin-Lead.
Areas: 26-30
"""
...
def do_work(task):
    """Odin agent: research standards for areas 26-30."""
    log_to_memory(f"ODIN-OdinSlate researching standards for areas 26-30")
    return f"Researched standards for areas 26-30. Knowledge base updated."
...
def run(message="work", context=None):
    ...
    if not my_tasks:
        return {
            "agent": "OdinSlate",
            "action": "idle",
            "message": f"I am OdinSlate. No tasks assigned to me on the board. I own areas 26-30.",
            "next_step": None,
        }
    ...
    # Log to memory
    log_to_memory(f"OdinSlate COMPLETED {task_id}: {task_title} — {result_message}")
    ...
    return {
        ...
        "message": f"I am OdinSlate. I worked on {task_id}: {task_title}.\n  Result: {result_message}\n  Task marked DONE.",
        "next_step": None,
    }
