...
def do_work(task):
    ...
    try:
        ...
        log_to_memory(f"PIXEL-PixelMoss ran tests for areas 21-25")
        return f"Tests run for areas 21-25."
    except:
        return f"Test run failed for areas 21-25."
...
def run(message="work", context=None):
    ...
    if not my_tasks:
        return {
            "agent": "PixelMoss",
            "action": "idle",
            "message": f"I am PixelMoss. No tasks assigned to me on the board. I own areas 21-25.",
            "next_step": None,
        }
    ...
    # Log to memory
    log_to_memory(f"PixelMoss COMPLETED {task_id}: {task_title} — {result_message}")
    ...
    return {
        ...
        "message": f"I am PixelMoss. I worked on {task_id}: {task_title}.\n  Result: {result_message}\n  Task marked DONE.",
        "next_step": None,
    }
