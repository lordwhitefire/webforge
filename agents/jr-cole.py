#!/usr/bin/env python3
"""
JrCole — Build Department
STANDALONE script. Checks board for assigned tasks and works on them.

Role: I am Jr-Cole. I am a Junior Frontend Developer. I report to my Senior Developer.
Areas: 16-20
"""
...
def do_work(task):
    ...
    log_to_memory(f"JrCole working on: {title}")
...
def run(message="work", context=None):
    ...
    if not my_tasks:
        return {
            "agent": "JrCole",
            "action": "idle",
            "message": f"I am JrCole. No tasks assigned to me on the board. I own areas 16-20.",
            "next_step": None,
        }
    ...
    return {
        ...
        "message": f"I am JrCole. I worked on {task_id}: {task_title}.\n  Result: {result_message}\n  Task marked DONE.",
        "next_step": None,
    }
