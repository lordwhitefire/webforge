#!/usr/bin/env python3
"""
JrVanadium — Build Department Worker

STANDALONE script. Does the actual work.

Role: I am JrVanadium. I am a Junior Frontend Developer. I report to Sr-Vance.
Areas: varies
"""
...
def log_to_memory(message: str, kind: str = "note"):
    try:
        ...
        session_append(message, agent="JrVanadium", kind=kind)
    except Exception:
        pass
...
def ack_task(task: dict):
    ...
    try:
        ...
        if assigned_msg:
            from mailbox import Mailbox
            Mailbox("jr-vanadium").ack(assigned_msg["id"], "On it — " + title)
    except Exception as e:
        _log(f"ack failed: {e}")
    ...
    notify_agent("Developer", "TASK_ACK",
                 "@JrVanadium ACK'd " + task_id + ": Junior Frontend Developer.", task_id)
    log_to_memory("JrVanadium ACK — picked up " + task_id + ": Junior Frontend Developer", kind="decision")
...
def run(message: str = "work", context: dict = None) -> dict:
    _log(f"JrVanadium triggered (task_id={os.environ.get('WEBFORGE_TASK_ID', '')!r})")
    ...
    if explicit_id:
        ...
        if explicit:
            ...
            if task and (task.get("owner") or "").lower() == "jr-vanadium":
                my_tasks = [task]

    if not my_tasks:
        no_work_msg = f"I am JrVanadium. No tasks assigned to me."
        _log(no_work_msg)
    ...
    try:
        notify_agent("Developer", "TASK_PROGRESS",
                     f"@JrVanadium → {task_id}: Starting work", task_id)
        checkpoint("working", {})
        result_message = do_work(task)
        notify_agent("Developer", "TASK_PROGRESS",
                     f"@JrVanadium → {task_id}: {result_message}", task_id)
        checkpoint("work_complete", {"result": result_message})
    except Exception as e:
        err_msg = f"Work failed: {e}"
        _log(err_msg)
        notify_agent("Developer", "TASK_BLOCKED",
                     f"@JrVanadium failed on {task_id}: {e}", task_id, priority=2)
        try:
        ...
        return {
            "agent": "JrVanadium", "action": "failed",
    ...
    notify_agent("Developer", "TASK_DONE",
                 f"@JrVanadium completed {task_id}: {task_title}. Result: {result_message}",
                 task_id)
    log_to_memory(f"JrVanadium COMPLETED {task_id}: {task_title} — {result_message}",
                  kind="decision")
    ...
    return {
        ...
        "message": (
            f"I am JrVanadium. I worked on {task_id}: {task_title}.\n"
...
if __name__ == "__main__":
    ...
    _log(f"=== JrVanadium starting (pid={os.getpid()}) ===")
    r = run(msg)
    _log(f"=== JrVanadium done: action={r.get('action')} ===")
