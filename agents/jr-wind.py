#!/usr/bin/env python3
"""
JrWind — Build Department Worker

STANDALONE script. Does the actual work.

Role: I am JrWind. I am a Junior Frontend Developer. I report to Sr-Brook.
Areas: varies
"""
...
def log_to_memory(message: str, kind: str = "note"):
    try:
        ...
        session_append(message, agent="JrWind", kind=kind)
    except Exception:
        pass
...
def ack_task(task: dict):
    ...
    try:
        ...
        if assigned_msg:
            from mailbox import Mailbox
            Mailbox("jr-wind").ack(assigned_msg["id"], "On it — " + title)
    except Exception as e:
        _log(f"ack failed: {e}")
    ...
    notify_agent("Developer", "TASK_ACK",
                 "@JrWind ACK'd " + task_id + ": Junior Frontend Developer.", task_id)
    log_to_memory("JrWind ACK — picked up " + task_id + ": Junior Frontend Developer", kind="decision")
...
def run(message: str = "work", context: dict = None) -> dict:
    _log(f"JrWind triggered (task_id={os.environ.get('WEBFORGE_TASK_ID', '')!r})")
    ...
    if explicit_id:
        ...
        if explicit:
            ...
            if task and (task.get("owner") or "").lower() == "jr-wind":
                my_tasks = [task]

    if not my_tasks:
        no_work_msg = f"I am JrWind. No tasks assigned to me."
        _log(no_work_msg)
    ...
    notify_agent("Developer", "TASK_DONE",
                 f"@JrWind completed {task_id}: {task_title}. Result: {result_message}",
                 task_id)
    log_to_memory(f"JrWind COMPLETED {task_id}: {task_title} — {result_message}",
                  kind="decision")
    ...
    return {
        ...
        "message": (
            f"I am JrWind. I worked on {task_id}: {task_title}.\n"
...
if __name__ == "__main__":
    ...
    _log(f"=== JrWind starting (pid={os.getpid()}) ===")
    r = run(msg)
    _log(f"=== JrWind done: action={r.get('action')} ===")
