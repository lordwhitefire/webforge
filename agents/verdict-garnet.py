...
def do_work(task):
    ...
    log_to_memory(f"VERDICT-VerdictGarnet checking standards for areas 21-25")
    return f"Standards check complete for areas 21-25."
...
def run(message="work", context=None):
    ...
    if not my_tasks:
        return {
            "agent": "VerdictGarnet",
            "action": "idle",
            "message": f"I am VerdictGarnet. No tasks assigned to me on the board. I own areas 21-25.",
            "next_step": None,
        }
    ...
    # Log to memory
    log_to_memory(f"VerdictGarnet COMPLETED {task_id}: {task_title} — {result_message}")
    ...
    return {
        ...
        "message": f"I am VerdictGarnet. I worked on {task_id}: {task_title}.\n  Result: {result_message}\n  Task marked DONE.",
        "next_step": None,
    }
