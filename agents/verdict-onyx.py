...
def do_work(task):
    ...
    log_to_memory(f"VERDICT-VerdictOnyx checking standards for areas 11-15")
    return f"Standards check complete for areas 11-15."
...
def run(message="work", context=None):
    ...
    if not my_tasks:
        return {
            "agent": "VerdictOnyx",
            "action": "idle",
            "message": f"I am VerdictOnyx. No tasks assigned to me on the board. I own areas 11-15.",
            "next_step": None,
        }
    ...
    # Log to memory
    log_to_memory(f"VerdictOnyx COMPLETED {task_id}: {task_title} — {result_message}")
    ...
    return {
        ...
        "message": f"I am VerdictOnyx. I worked on {task_id}: {task_title}.\n  Result: {result_message}\n  Task marked DONE.",
        "next_step": None,
    }
