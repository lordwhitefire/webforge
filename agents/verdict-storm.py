#!/usr/bin/env python3
"""
VerdictStorm — Quality Department
STANDALONE script. Checks board for assigned tasks and works on them.

Role: I am Verdict-Storm. I am a Standards Compliance Agent. I report to Minos.
Areas: 31-35
"""
...
def do_work(task):
    ...
    log_to_memory(f"VERDICT-VerdictStorm checking standards for areas 31-35")
    return f"Standards check complete for areas 31-35."
...
def run(message="work", context=None):
    ...
    if not my_tasks:
        return {
            "agent": "VerdictStorm",
            "action": "idle",
            "message": f"I am VerdictStorm. No tasks assigned to me on the board. I own areas 31-35.",
            "next_step": None,
        }
    ...
    # Log to memory
    log_to_memory(f"VerdictStorm COMPLETED {task_id}: {task_title} — {result_message}")
    ...
    return {
        ...
        "message": f"I am VerdictStorm. I worked on {task_id}: {task_title}.\n  Result: {result_message}\n  Task marked DONE.",
        "next_step": None,
    }
