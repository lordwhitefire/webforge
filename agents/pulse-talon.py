#!/usr/bin/env python3
"""
PulseTalon — Quality Department
STANDALONE script. Checks board for assigned tasks and works on them.

Role: I am Pulse-Talon. I am a Bug Fixer Agent. I report to Pulse-Core.
Areas: 56-60
"""
...
def do_work(task):
    ...
    log_to_memory(f"PULSE-PulseTalon fixing: {title}")
...
def run(message="work", context=None):
    ...
    if not my_tasks:
        return {
            "agent": "PulseTalon",
            "action": "idle",
            "message": f"I am PulseTalon. No tasks assigned to me on the board. I own areas 56-60.",
            "next_step": None,
        }
    ...
    # Log to memory
    log_to_memory(f"PulseTalon COMPLETED {task_id}: {task_title} — {result_message}")
    ...
    return {
        ...
        "message": f"I am PulseTalon. I worked on {task_id}: {task_title}.\n  Result: {result_message}\n  Task marked DONE.",
        "next_step": None,
    }
