#!/usr/bin/env python3
"""
PulseCove — Quality Department
STANDALONE script. Checks board for assigned tasks and works on them.

Role: I am Pulse-Cove. I am a Bug Fixer Agent. I report to Pulse-Core.
Areas: 46-50
"""
...
def do_work(task):
    ...
    log_to_memory(f"PULSE-PulseCove fixing: {title}")
...
def run(message="work", context=None):
    ...
    if not my_tasks:
        return {
            "agent": "PulseCove",
            "action": "idle",
            "message": f"I am PulseCove. No tasks assigned to me on the board. I own areas 46-50.",
            "next_step": None,
        }
    ...
    # Log to memory
    log_to_memory(f"PulseCove COMPLETED {task_id}: {task_title} — {result_message}")
    ...
    return {
        ...
        "message": f"I am PulseCove. I worked on {task_id}: {task_title}.\n  Result: {result_message}\n  Task marked DONE.",
        "next_step": None,
    }
