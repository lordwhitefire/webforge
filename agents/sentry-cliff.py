...
def run(message="work", context=None):
    ...
    if not my_tasks:
        return {
            "agent": "SentryCliff",
            "action": "idle",
            "message": f"I am SentryCliff. No tasks assigned to me on the board. I own areas 16-20.",
            "next_step": None,
        }
    ...
    # Log to memory
    log_to_memory(f"SentryCliff COMPLETED {task_id}: {task_title} — {result_message}")
    ...
    return {
        ...
        "message": f"I am SentryCliff. I worked on {task_id}: {task_title}.\n  Result: {result_message}\n  Task marked DONE.",
        "next_step": None,
    }
