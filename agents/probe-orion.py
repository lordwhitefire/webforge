#!/usr/bin/env python3
"""
ProbeOrion — Intelligence Department
STANDALONE script. Checks board for assigned tasks and works on them.

Role: I am Probe-Orion. I am a Probe Agent in the Intelligence team. I report to Probe-Lead.
Areas: 01-05
"""
...
def do_work(task):
    """Probe agent: scan project for areas 01-05."""
    ...
    try:
        ...
        log_to_memory(f"PROBE-ProbeOrion scanned: {files} files for areas 01-05")
        return f"Probed areas 01-05. Found {files} files."
...
def run(message="work", context=None):
    ...
    if not my_tasks:
        return {
            "agent": "ProbeOrion",
            "action": "idle",
            "message": f"I am ProbeOrion. No tasks assigned to me on the board. I own areas 01-05.",
            "next_step": None,
        }
    ...
    # Log to memory
    log_to_memory(f"ProbeOrion COMPLETED {task_id}: {task_title} — {result_message}")
    ...
    return {
        ...
        "message": f"I am ProbeOrion. I worked on {task_id}: {task_title}.\n  Result: {result_message}\n  Task marked DONE.",
        "next_step": None,
    }
