...
def do_work(task):
    ...
    try:
        ...
        log_to_memory(f"PIXEL-PixelFern ran tests for areas 31-35")
        return f"Tests run for areas 31-35."
    except:
        return f"Test run failed for areas 31-35."
...
def run(message="work", context=None):
    ...
    if not my_tasks:
        return {
            "agent": "PixelFern",
            "action": "idle",
            "message": f"I am PixelFern. No tasks assigned to me on the board. I own areas 31-35.",
            "next_step": None,
        }
    ...
    # Log to memory
    log_to_memory(f"PixelFern COMPLETED {task_id}: {task_title} — {result_message}")
    ...
    return {
        ...
        "message": f"I am PixelFern. I worked on {task_id}: {task_title}.\n  Result: {result_message}\n  Task marked DONE.",
        "next_step": None,
    }
