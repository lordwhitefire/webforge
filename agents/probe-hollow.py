...
Role: I am Probe-Hollow. I am a Probe Agent in the Intelligence team. I report to Probe-Lead.
Areas: 61-65
"""
...
def do_work(task):
    """Probe agent: scan project for areas 61-65."""
    ...
    try:
        ...
        log_to_memory(f"PROBE-ProbeHollow scanned: {files} files for areas 61-65")
        return f"Probed areas 61-65. Found {files} files."
...
def run(message="work", context=None):
    ...
    if not my_tasks:
        return {
            "agent": "ProbeHollow",
            "action": "idle",
            "message": f"I am ProbeHollow. No tasks assigned to me on the board. I own areas 61-65.",
            "next_step": None,
        }
    ...
    # Log to memory
    log_to_memory(f"ProbeHollow COMPLETED {task_id}: {task_title} — {result_message}")
    ...
    return {
        ...
        "message": f"I am ProbeHollow. I worked on {task_id}: {task_title}.\n  Result: {result_message}\n  Task marked DONE.",
        "next_step": None,
    }
