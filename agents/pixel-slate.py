#!/usr/bin/env python3
"""
PixelSlate — Quality Department
STANDALONE script. Checks board for assigned tasks and works on them.

Role: I am Pixel-Slate. I am a I write and run unit/integration tests agent. I report to Pixel-Core.
Areas: 26-30
"""
...
def do_work(task):
    ...
    try:
        ...
        log_to_memory(f"PIXEL-PixelSlate ran tests for areas 26-30")
        return f"Tests run for areas 26-30."
    except:
        return f"Test run failed for areas 26-30."
...
def run(message="work", context=None):
    ...
    if not my_tasks:
        return {
            "agent": "PixelSlate",
            "action": "idle",
            "message": f"I am PixelSlate. No tasks assigned to me on the board. I own areas 26-30.",
            "next_step": None,
        }
    ...
    # Log to memory
    log_to_memory(f"PixelSlate COMPLETED {task_id}: {task_title} — {result_message}")
    ...
    return {
        ...
        "message": f"I am PixelSlate. I worked on {task_id}: {task_title}.\n  Result: {result_message}\n  Task marked DONE.",
        "next_step": None,
    }
