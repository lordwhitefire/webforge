...
def do_work(task):
    ...
    log_to_memory(f"VERDICT-VerdictWren2 checking standards for areas 81-82")
    return f"Standards check complete for areas 81-82."
...
def run(message="work", context=None):
    ...
    if not my_tasks:
        return {
            "agent": "VerdictWren2",
            "action": "idle",
            "message": f"I am VerdictWren2. No tasks assigned to me on the board. I own areas 81-82.",
            "next_step": None,
        }
    ...
    # Log to memory
    log_to_memory(f"VerdictWren2 COMPLETED {task_id}: {task_title} — {result_message}")
    ...
    return {
        ...
        "message": f"I am VerdictWren2. I worked on {task_id}: {task_title}.\n  Result: {result_message}\n  Task marked DONE.",
        "next_step": None,
    }
