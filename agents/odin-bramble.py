#!/usr/bin/env python3
"""
OdinBramble — Intelligence Department
STANDALONE script. Checks board for assigned tasks and works on them.

Role: I am Odin-Bramble. I am an Odin Agent in the Intelligence team. I report to Odin-Lead.
Areas: 51-55
"""
...
def do_work(task):
    """Odin agent: research standards for areas 51-55."""
    log_to_memory(f"ODIN-OdinBramble researching standards for areas 51-55")
    return f"Researched standards for areas 51-55. Knowledge base updated."
...
def run(message="work", context=None):
    ...
    if not my_tasks:
        return {
            "agent": "OdinBramble",
            "action": "idle",
            "message": f"I am OdinBramble. No tasks assigned to me on the board. I own areas 51-55.",
            "next_step": None,
        }
    ...
    # Log to memory
    log_to_memory(f"OdinBramble COMPLETED {task_id}: {task_title} — {result_message}")
    ...
    return {
        ...
        "message": f"I am OdinBramble. I worked on {task_id}: {task_title}.\n  Result: {result_message}\n  Task marked DONE.",
        "next_step": None,
    }
