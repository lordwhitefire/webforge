...
def do_work(task):
    ...
    log_to_memory(f"JANUS-JanusWisp security check for areas 41-45")
    return f"Security check complete for areas 41-45."
...
def run(message="work", context=None):
    ...
    if not my_tasks:
        return {
            "agent": "JanusWisp",
            "action": "idle",
            "message": f"I am JanusWisp. No tasks assigned to me on the board. I own areas 41-45.",
            "next_step": None,
        }
    ...
    # Log to memory
    log_to_memory(f"JanusWisp COMPLETED {task_id}: {task_title} — {result_message}")
    ...
    return {
        ...
        "message": f"I am JanusWisp. I worked on {task_id}: {task_title}.\n  Result: {result_message}\n  Task marked DONE.",
        "next_step": None,
    }
