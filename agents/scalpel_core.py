...
def run(message="work", context=None):
    ...
    if not my_tasks:
        return {
            "agent": "ScalpelCore",
            "action": "idle",
            "message": f"I am ScalpelCore. No tasks assigned to me on the board. I own areas assigned.",
            "next_step": None,
        }
    ...
    # Log to memory
    log_to_memory(f"ScalpelCore COMPLETED {task_id}: {task_title} — {result_message}")
    ...
    return {
        ...
        "message": f"I am ScalpelCore. I worked on {task_id}: {task_title}.\n  Result: {result_message}\n  Task marked DONE.",
        "next_step": None,
    }
