#!/usr/bin/env python3
"""
Anvil — Meta Department
STANDALONE script. Checks board for assigned tasks and works on them.

Role: I am Anvil. I am the MCP Fixer. I report to Daedalus.
Areas: N/A
"""
...
def run(message="work", context=None):
    ...
    if not my_tasks:
        return {
            "agent": "Anvil",
            "action": "idle",
            "message": f"I am Anvil. No tasks assigned to me on the board. I own areas assigned.",
            "next_step": None,
        }
    ...
    # Log to memory
    log_to_memory(f"Anvil COMPLETED {task_id}: {task_title} — {result_message}")
    ...
    return {
        ...
        "message": f"I am Anvil. I worked on {task_id}: {task_title}.\n  Result: {result_message}\n  Task marked DONE.",
        "next_step": None,
    }
