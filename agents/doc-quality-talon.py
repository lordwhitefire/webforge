#!/usr/bin/env python3
"""
DocQualityTalon — Documentation Department
STANDALONE script. Checks board for assigned tasks and works on them.

Role: I am Doc-Quality-Talon. I am an Embedded Documentation Agent. I report to Thoth.
Areas: N/A
"""
...
def do_work(task):
    ...
    log_to_memory(f"DOC-DocQualityTalon recording for areas ")
...
def run(message="work", context=None):
    ...
    if not my_tasks:
        return {
            "agent": "DocQualityTalon",
            "action": "idle",
            "message": f"I am DocQualityTalon. No tasks assigned to me on the board. I own areas assigned.",
            "next_step": None,
        }
    ...
    # Log to memory
    log_to_memory(f"DocQualityTalon COMPLETED {task_id}: {task_title} — {result_message}")
    ...
    return {
        ...
        "message": f"I am DocQualityTalon. I worked on {task_id}: {task_title}.\n  Result: {result_message}\n  Task marked DONE.",
        "next_step": None,
    }
