...
Areas: 56-60
Skill file: skills/intelligence/probe-brisk.md
"""
...
def run(message, context=None):
    """Probe agent: scans project for areas 56-60."""
    ...
    try:
        subprocess.run(["python3", str(MCP_DIR / "memory.py"), "session-append",
                       f"PROBE-ProbeBrisk scanned project. Files: {scan_data.get('stats', {}).get('total_files', '?')}",
                       "ProbeBrisk", "note"],
    ...
    return {
        ...
        "action": "probe",
        "areas": "56-60",
        "message": f"I am ProbeBrisk. I probed areas 56-60. Found {scan_data.get('stats', {}).get('total_files', '?')} files. Findings written to memory.",
        "scan": scan_data,
        "next_step": None,
    }
