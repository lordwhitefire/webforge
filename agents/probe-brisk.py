...
def do_work(task):
    """Probe agent: scan project for areas 56-60."""
    ...
    try:
        ...
        log_to_memory(f"PROBE-ProbeBrisk scanned: {files} files for areas 56-60")
        return f"Probed areas 56-60. Found {files} files."
...
def run(message="work", context=None):
    ...
    if not my_tasks:
        return {
            "agent": "ProbeBrisk",
            "action": "idle",
            "message": f"I am ProbeBrisk. No tasks assigned to me on the board. I own areas 56-60.",
            "next_step": None,
        }
    ...
    # Log to memory
    log_to_memory(f"ProbeBrisk COMPLETED {task_id}: {task_title} — {result_message}")
    ...
    return {
        ...
        "message": f"I am ProbeBrisk. I worked on {task_id}: {task_title}.\n  Result: {result_message}\n  Task marked DONE.",
        "next_step": None,
    }
