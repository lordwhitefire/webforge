#!/usr/bin/env python3
"""
OdinAster — Intelligence Department
STANDALONE script. Checks board for assigned tasks and works on them.

Role: I am Odin-Aster. I am an Odin Agent in the Intelligence team. I report to Odin-Lead.
Areas: 81-82
"""
...
def do_work(task):
    """Odin agent: research standards for areas 81-82."""
    log_to_memory(f"ODIN-OdinAster researching standards for areas 81-82")
    return f"Researched standards for areas 81-82. Knowledge base updated."
...
def run(message="work", context=None):
    ...
    if not my_tasks:
        return {
            "agent": "OdinAster",
            "action": "idle",
            "message": f"I am OdinAster. No tasks assigned to me on the board. I own areas 81-82.",
            "next_step": None,
        }
    ...
    # Log to memory
    log_to_memory(f"OdinAster COMPLETED {task_id}: {task_title} — {result_message}")
    ...
    return {
        ...
        "message": f"I am OdinAster. I worked on {task_id}: {task_title}.\n  Result: {result_message}\n  Task marked DONE.",
        "next_step": None,
    }
