#!/usr/bin/env python3
"""
SentryBirch — Quality Department
STANDALONE script. Checks board for assigned tasks and works on them.

Role: I am Sentry-Birch. I am a I review test scripts written by Pixel agents agent. I report to Sentry-Core.
Areas: 11-15
"""
...
def run(message="work", context=None):
    ...
    if not my_tasks:
        return {
            "agent": "SentryBirch",
            "action": "idle",
            "message": f"I am SentryBirch. No tasks assigned to me on the board. I own areas 11-15.",
            "next_step": None,
        }
    ...
    # Log to memory
    log_to_memory(f"SentryBirch COMPLETED {task_id}: {task_title} — {result_message}")
    ...
    return {
        ...
        "message": f"I am SentryBirch. I worked on {task_id}: {task_title}.\n  Result: {result_message}\n  Task marked DONE.",
        "next_step": None,
    }
