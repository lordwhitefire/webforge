#!/usr/bin/env python3
"""
PixelBirch — Quality Department
STANDALONE script. Checks board for assigned tasks and works on them.

Role: I am Pixel-Birch. I am a I write and run unit/integration tests agent. I report to Pixel-Core.
Areas: 11-15
"""
...
def do_work(task):
    ...
    try:
        ...
        log_to_memory(f"PIXEL-PixelBirch ran tests for areas 11-15")
        return f"Tests run for areas 11-15."
    except:
        return f"Test run failed for areas 11-15."
...
def run(message="work", context=None):
    ...
    if not my_tasks:
        return {
            "agent": "PixelBirch",
            "action": "idle",
            "message": f"I am PixelBirch. No tasks assigned to me on the board. I own areas 11-15.",
            "next_step": None,
        }
    ...
    # Log to memory
    log_to_memory(f"PixelBirch COMPLETED {task_id}: {task_title} — {result_message}")
    ...
    return {
        ...
        "message": f"I am PixelBirch. I worked on {task_id}: {task_title}.\n  Result: {result_message}\n  Task marked DONE.",
        "next_step": None,
    }
