#!/usr/bin/env python3
"""
Stamp — Documentation Department
STANDALONE script. Checks board for assigned tasks and works on them.

Role: I am Stamp. I manage commit messages and progress tracking. I report to Quill.
Areas: N/A
"""
...
def run(message="work", context=None):
    ...
    if not my_tasks:
        return {
            "agent": "Stamp",
            "action": "idle",
            "message": f"I am Stamp. No tasks assigned to me on the board. I own areas assigned.",
            "next_step": None,
        }
    ...
    # Log to memory
    log_to_memory(f"Stamp COMPLETED {task_id}: {task_title} — {result_message}")
    ...
    return {
        ...
        "message": f"I am Stamp. I worked on {task_id}: {task_title}.\n  Result: {result_message}\n  Task marked DONE.",
        "next_step": None,
    }
