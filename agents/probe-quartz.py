...
Role: I am Probe-Quartz. I am a Probe Agent in the Intelligence team. I report to Probe-Lead.
Areas: 21-25
"""
...
def do_work(task):
    """Probe agent: scan project for areas 21-25."""
    ...
    try:
        ...
        log_to_memory(f"PROBE-ProbeQuartz scanned: {files} files for areas 21-25")
        return f"Probed areas 21-25. Found {files} files."
...
def run(message="work", context=None):
    ...
    if not my_tasks:
        return {
            "agent": "ProbeQuartz",
            "action": "idle",
            "message": f"I am ProbeQuartz. No tasks assigned to me on the board. I own areas 21-25.",
            "next_step": None,
        }
    ...
    # Log to memory
    log_to_memory(f"ProbeQuartz COMPLETED {task_id}: {task_title} — {result_message}")
    ...
    return {
        ...
        "message": f"I am ProbeQuartz. I worked on {task_id}: {task_title}.\n  Result: {result_message}\n  Task marked DONE.",
        "next_step": None,
    }
