...
Role: I am JrWillow. I am a JrWillow. I report to Aurora.
Areas: 31-35
"""
...
def ack_task(task: dict):
    ...
    try:
        ...
        my_def = get_agent("jr-willow")
        superior = my_def.reports_to if my_def and my_def.reports_to else "Hephaestus"
    except ImportError:
        superior = "Hephaestus"
    notify_agent(superior, "TASK_ACK",
                 "ACK — picked up " + task_id + ": JrWillow.", task_id)
    notify_agent("Developer", "TASK_ACK",
                 "@JrWillow ACK'd " + task_id + ": JrWillow.", task_id)
    log_to_memory("JrWillow ACK — picked up " + task_id + ": JrWillow", kind="decision")
