#!/usr/bin/env python3
"""
DocBuildFern — Documentation Department
STANDALONE script. Checks board for assigned tasks and works on them.

Role: I am Doc-Build-Fern. I am an Embedded Documentation Agent. I report to Thoth.
Areas: N/A
"""
...
def do_work(task):
    ...
    log_to_memory(f"DOC-DocBuildFern recording for areas ")
...
def run(message="work", context=None):
    ...
    if not my_tasks:
        return {
            "agent": "DocBuildFern",
            "action": "idle",
            "message": f"I am DocBuildFern. No tasks assigned to me on the board. I own areas assigned.",
            "next_step": None,
        }
    ...
    # Log to memory
    log_to_memory(f"DocBuildFern COMPLETED {task_id}: {task_title} — {result_message}")
    ...
    return {
        ...
        "message": f"I am DocBuildFern. I worked on {task_id}: {task_title}.\n  Result: {result_message}\n  Task marked DONE.",
        "next_step": None,
    }
