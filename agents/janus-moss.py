#!/usr/bin/env python3
"""
JanusMoss — Quality Department
STANDALONE script. Checks board for assigned tasks and works on them.

Role: I am Janus-Moss. I am a Security & Compliance Agent. I report to Janus-Core.
Areas: 21-25
"""
...
def do_work(task):
    ...
    log_to_memory(f"JANUS-JanusMoss security check for areas 21-25")
    return f"Security check complete for areas 21-25."
...
def run(message="work", context=None):
    ...
    if not my_tasks:
        return {
            "agent": "JanusMoss",
            "action": "idle",
            "message": f"I am JanusMoss. No tasks assigned to me on the board. I own areas 21-25.",
            "next_step": None,
        }
    ...
    # Log to memory
    log_to_memory(f"JanusMoss COMPLETED {task_id}: {task_title} — {result_message}")
    ...
    return {
        ...
        "message": f"I am JanusMoss. I worked on {task_id}: {task_title}.\n  Result: {result_message}\n  Task marked DONE.",
        "next_step": None,
    }
