#!/usr/bin/env python3
"""
PulseBramble — Quality Department
STANDALONE script. Checks board for assigned tasks and works on them.

Role: I am Pulse-Bramble. I am a Bug Fixer Agent. I report to Pulse-Core.
Areas: 51-55
"""
...
def do_work(task):
    ...
    log_to_memory(f"PULSE-PulseBramble fixing: {title}")
...
def run(message="work", context=None):
    ...
    if not my_tasks:
        return {
            "agent": "PulseBramble",
            "action": "idle",
            "message": f"I am PulseBramble. No tasks assigned to me on the board. I own areas 51-55.",
            "next_step": None,
        }
    ...
    # Log to memory
    log_to_memory(f"PulseBramble COMPLETED {task_id}: {task_title} — {result_message}")
    ...
    return {
        ...
        "message": f"I am PulseBramble. I worked on {task_id}: {task_title}.\n  Result: {result_message}\n  Task marked DONE.",
        "next_step": None,
    }
