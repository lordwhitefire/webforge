...
def do_work(task):
    """Odin agent: research standards for areas 76-80."""
    log_to_memory(f"ODIN-OdinFrost researching standards for areas 76-80")
    return f"Researched standards for areas 76-80. Knowledge base updated."
...
def run(message="work", context=None):
    ...
    if not my_tasks:
        return {
            "agent": "OdinFrost",
            "action": "idle",
            "message": f"I am OdinFrost. No tasks assigned to me on the board. I own areas 76-80.",
            "next_step": None,
        }
    ...
    # Log to memory
    log_to_memory(f"OdinFrost COMPLETED {task_id}: {task_title} — {result_message}")
    ...
    return {
        ...
        "message": f"I am OdinFrost. I worked on {task_id}: {task_title}.\n  Result: {result_message}\n  Task marked DONE.",
        "next_step": None,
    }
