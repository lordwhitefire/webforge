#!/usr/bin/env python3
"""
SrWater — Build Department
STANDALONE script. Checks board for assigned tasks and works on them.

Role: I am Sr-Water. I am a Senior Developer in the DB/Infra sub-department. I report to Lead-Zen.
Areas: N/A
"""
...
def do_work(task):
    ...
    log_to_memory(f"SR-SrWater reviewing: {title}")
...
def run(message="work", context=None):
    ...
    if not my_tasks:
        return {
            "agent": "SrWater",
            "action": "idle",
            "message": f"I am SrWater. No tasks assigned to me on the board. I own areas assigned.",
            "next_step": None,
        }
    ...
    # Log to memory
    log_to_memory(f"SrWater COMPLETED {task_id}: {task_title} — {result_message}")
    ...
    return {
        ...
        "message": f"I am SrWater. I worked on {task_id}: {task_title}.\n  Result: {result_message}\n  Task marked DONE.",
        "next_step": None,
    }
