#!/usr/bin/env python3
"""
PixelAster — Quality Department
STANDALONE script. Checks board for assigned tasks and works on them.

Role: I am Pixel-Aster. I am a I write and run unit/integration tests agent. I report to Pixel-Core.
Areas: 81-82
"""
...
def do_work(task):
    ...
    try:
        ...
        log_to_memory(f"PIXEL-PixelAster ran tests for areas 81-82")
        return f"Tests run for areas 81-82."
    except:
        return f"Test run failed for areas 81-82."
...
def run(message="work", context=None):
    ...
    if not my_tasks:
        return {
            "agent": "PixelAster",
            "action": "idle",
            "message": f"I am PixelAster. No tasks assigned to me on the board. I own areas 81-82.",
            "next_step": None,
        }
    ...
    # Log to memory
    log_to_memory(f"PixelAster COMPLETED {task_id}: {task_title} — {result_message}")
    ...
    return {
        ...
        "message": f"I am PixelAster. I worked on {task_id}: {task_title}.\n  Result: {result_message}\n  Task marked DONE.",
        "next_step": None,
    }
