#!/usr/bin/env python3
"""
JrAsh — Build Department Worker

STANDALONE script. Does the actual work.

Role: I am JrAsh. I am a Junior Database/Infra Developer. I report to Sr-Water.
Areas: 51-55
"""
...
def log_to_memory(message: str, kind: str = "note"):
    try:
        ...
        session_append(message, agent="JrAsh", kind=kind)
    except Exception:
        pass
...
def ack_task(task: dict):
    ...
    try:
        ...
        if assigned_msg:
            from mailbox import Mailbox
            Mailbox("jr-ash").ack(assigned_msg["id"], "On it — " + title)
    except Exception as e:
        _log(f"ack failed: {e}")
    ...
    notify_agent("Developer", "TASK_ACK",
                 "@JrAsh ACK'd " + task_id + ": Junior Database/Infra Developer.", task_id)
    log_to_memory("JrAsh ACK — picked up " + task_id + ": Junior Database/Infra Developer", kind="decision")
...
def run(message: str = "work", context: dict = None) -> dict:
    _log(f"JrAsh triggered (task_id={os.environ.get('WEBFORGE_TASK_ID', '')!r})")
    ...
    if explicit_id:
        ...
        if explicit:
            ...
            if task and (task.get("owner") or "").lower() == "jr-ash":
                my_tasks = [task]

    if not my_tasks:
        no_work_msg = f"I am JrAsh. No tasks assigned to me."
        _log(no_work_msg)
    ...
    notify_agent("Developer", "TASK_DONE",
                 f"@JrAsh completed {task_id}: {task_title}. Result: {result_message}",
                 task_id)
    log_to_memory(f"JrAsh COMPLETED {task_id}: {task_title} — {result_message}",
                  kind="decision")
    ...
    return {
        ...
        "message": (
            f"I am JrAsh. I worked on {task_id}: {task_title}.\n"
...
if __name__ == "__main__":
    ...
    _log(f"=== JrAsh starting (pid={os.getpid()}) ===")
    r = run(msg)
    _log(f"=== JrAsh done: action={r.get('action')} ===")
