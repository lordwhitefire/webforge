#!/usr/bin/env python3
"""
ProbeMarsh — Intelligence Department
STANDALONE script. Checks board for assigned tasks and works on them.

Role: I am Probe-Marsh. I am a Probe Agent in the Intelligence team. I report to Probe-Lead.
Areas: 36-40
"""
...
def do_work(task):
    """Probe agent: scan project for areas 36-40."""
    ...
    try:
        ...
        log_to_memory(f"PROBE-ProbeMarsh scanned: {files} files for areas 36-40")
        return f"Probed areas 36-40. Found {files} files."
...
def run(message="work", context=None):
    ...
    if not my_tasks:
        return {
            "agent": "ProbeMarsh",
            "action": "idle",
            "message": f"I am ProbeMarsh. No tasks assigned to me on the board. I own areas 36-40.",
            "next_step": None,
        }
    ...
    # Log to memory
    log_to_memory(f"ProbeMarsh COMPLETED {task_id}: {task_title} — {result_message}")
    ...
    return {
        ...
        "message": f"I am ProbeMarsh. I worked on {task_id}: {task_title}.\n  Result: {result_message}\n  Task marked DONE.",
        "next_step": None,
    }
