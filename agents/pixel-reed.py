#!/usr/bin/env python3
"""
PixelReed — Quality Department
STANDALONE script. Checks board for assigned tasks and works on them.

Role: I am Pixel-Reed. I am a I write and run unit/integration tests agent. I report to Pixel-Core.
Areas: 06-10
"""
...
def do_work(task):
    ...
    try:
        ...
        log_to_memory(f"PIXEL-PixelReed ran tests for areas 06-10")
        return f"Tests run for areas 06-10."
    except:
        return f"Test run failed for areas 06-10."
...
def run(message="work", context=None):
    ...
    if not my_tasks:
        return {
            "agent": "PixelReed",
            "action": "idle",
            "message": f"I am PixelReed. No tasks assigned to me on the board. I own areas 06-10.",
            "next_step": None,
        }
    ...
    # Log to memory
    log_to_memory(f"PixelReed COMPLETED {task_id}: {task_title} — {result_message}")
    ...
    return {
        ...
        "message": f"I am PixelReed. I worked on {task_id}: {task_title}.\n  Result: {result_message}\n  Task marked DONE.",
        "next_step": None,
    }
