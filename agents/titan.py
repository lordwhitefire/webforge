#!/usr/bin/env python3
"""
Titan — Build Department
STANDALONE script. Checks board for assigned tasks and works on them.

Role: I am Titan. I lead the Backend sub-department. I report to Hephaestus.
Areas: N/A
"""
...
def run(message="work", context=None):
    ...
    if not my_tasks:
        return {
            "agent": "Titan",
            "action": "idle",
            "message": f"I am Titan. No tasks assigned to me on the board. I own areas assigned.",
            "next_step": None,
        }
    ...
    # Log to memory
    log_to_memory(f"Titan COMPLETED {task_id}: {task_title} — {result_message}")
    ...
    return {
        ...
        "message": f"I am Titan. I worked on {task_id}: {task_title}.\n  Result: {result_message}\n  Task marked DONE.",
        "next_step": None,
    }
