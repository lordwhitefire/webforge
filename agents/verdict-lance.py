...
def do_work(task):
    ...
    log_to_memory(f"VERDICT-VerdictLance checking standards for areas 01-05")
    return f"Standards check complete for areas 01-05."
...
def run(message="work", context=None):
    ...
    if not my_tasks:
        return {
            "agent": "VerdictLance",
            "action": "idle",
            "message": f"I am VerdictLance. No tasks assigned to me on the board. I own areas 01-05.",
            "next_step": None,
        }
    ...
    # Log to memory
    log_to_memory(f"VerdictLance COMPLETED {task_id}: {task_title} — {result_message}")
    ...
    return {
        ...
        "message": f"I am VerdictLance. I worked on {task_id}: {task_title}.\n  Result: {result_message}\n  Task marked DONE.",
        "next_step": None,
    }
