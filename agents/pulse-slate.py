...
def do_work(task):
    ...
    log_to_memory(f"PULSE-PulseSlate fixing: {title}")
...
def run(message="work", context=None):
    ...
    if not my_tasks:
        return {
            "agent": "PulseSlate",
            "action": "idle",
            "message": f"I am PulseSlate. No tasks assigned to me on the board. I own areas 26-30.",
            "next_step": None,
        }
    ...
    # Log to memory
    log_to_memory(f"PulseSlate COMPLETED {task_id}: {task_title} — {result_message}")
    ...
    return {
        ...
        "message": f"I am PulseSlate. I worked on {task_id}: {task_title}.\n  Result: {result_message}\n  Task marked DONE.",
        "next_step": None,
    }
