#!/usr/bin/env python3
"""
DocIntelligenceMoss — Documentation Department
STANDALONE script. Checks board for assigned tasks and works on them.

Role: I am Doc-Intelligence-Moss. I am an Embedded Documentation Agent. I report to Thoth.
Areas: N/A
"""
...
def log_to_memory(message):
    ...
    try:
        subprocess.run(["python3", str(MCP_DIR / "memory.py"), "session-append",
                       message, "DocIntelligenceMoss", "note"],
...
def do_work(task):
    ...
    log_to_memory(f"DOC-DocIntelligenceMoss recording for areas ")
...
def run(message="work", context=None):
    ...
    if not my_tasks:
        return {
            "agent": "DocIntelligenceMoss",
            "action": "idle",
            "message": f"I am DocIntelligenceMoss. No tasks assigned to me on the board. I own areas assigned.",
            "next_step": None,
        }
    ...
    # Log to memory
    log_to_memory(f"DocIntelligenceMoss COMPLETED {task_id}: {task_title} — {result_message}")
    ...
    return {
        ...
        "message": f"I am DocIntelligenceMoss. I worked on {task_id}: {task_title}.\n  Result: {result_message}\n  Task marked DONE.",
        "next_step": None,
    }
