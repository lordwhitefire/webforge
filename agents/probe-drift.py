...
Role: I am Probe-Drift. I am a Probe Agent in the Intelligence team. I report to Probe-Lead.
Areas: 71-75
"""
...
def do_work(task):
    """Probe agent: scan project for areas 71-75."""
    ...
    try:
        ...
        log_to_memory(f"PROBE-ProbeDrift scanned: {files} files for areas 71-75")
        return f"Probed areas 71-75. Found {files} files."
...
def run(message="work", context=None):
    ...
    if not my_tasks:
        return {
            "agent": "ProbeDrift",
            "action": "idle",
            "message": f"I am ProbeDrift. No tasks assigned to me on the board. I own areas 71-75.",
            "next_step": None,
        }
    ...
    # Log to memory
    log_to_memory(f"ProbeDrift COMPLETED {task_id}: {task_title} — {result_message}")
    ...
    return {
        ...
        "message": f"I am ProbeDrift. I worked on {task_id}: {task_title}.\n  Result: {result_message}\n  Task marked DONE.",
        "next_step": None,
    }
