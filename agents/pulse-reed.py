#!/usr/bin/env python3
"""
PulseReed — Quality Department
STANDALONE script. Checks board for assigned tasks and works on them.

Role: I am Pulse-Reed. I am a Bug Fixer Agent. I report to Pulse-Core.
Areas: 06-10
"""
...
def do_work(task):
    ...
    log_to_memory(f"PULSE-PulseReed fixing: {title}")
...
def run(message="work", context=None):
    ...
    if not my_tasks:
        return {
            "agent": "PulseReed",
            "action": "idle",
            "message": f"I am PulseReed. No tasks assigned to me on the board. I own areas 06-10.",
            "next_step": None,
        }
    ...
    # Log to memory
    log_to_memory(f"PulseReed COMPLETED {task_id}: {task_title} — {result_message}")
    ...
    return {
        ...
        "message": f"I am PulseReed. I worked on {task_id}: {task_title}.\n  Result: {result_message}\n  Task marked DONE.",
        "next_step": None,
    }
