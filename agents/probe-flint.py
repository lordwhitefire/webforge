#!/usr/bin/env python3
"""
ProbeFlint — Intelligence Department
STANDALONE script. Checks board for assigned tasks and works on them.

Role: I am Probe-Flint. I am a Probe Agent in the Intelligence team. I report to Probe-Lead.
Areas: 26-30
"""
...
def do_work(task):
    """Probe agent: scan project for areas 26-30."""
    ...
    try:
        ...
        log_to_memory(f"PROBE-ProbeFlint scanned: {files} files for areas 26-30")
        return f"Probed areas 26-30. Found {files} files."
...
def run(message="work", context=None):
    ...
    if not my_tasks:
        return {
            "agent": "ProbeFlint",
            "action": "idle",
            "message": f"I am ProbeFlint. No tasks assigned to me on the board. I own areas 26-30.",
            "next_step": None,
        }
    ...
    # Log to memory
    log_to_memory(f"ProbeFlint COMPLETED {task_id}: {task_title} — {result_message}")
    ...
    return {
        ...
        "message": f"I am ProbeFlint. I worked on {task_id}: {task_title}.\n  Result: {result_message}\n  Task marked DONE.",
        "next_step": None,
    }
