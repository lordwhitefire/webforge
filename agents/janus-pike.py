...
def do_work(task):
    ...
    log_to_memory(f"JANUS-JanusPike security check for areas 36-40")
    return f"Security check complete for areas 36-40."
...
def run(message="work", context=None):
    ...
    if not my_tasks:
        return {
            "agent": "JanusPike",
            "action": "idle",
            "message": f"I am JanusPike. No tasks assigned to me on the board. I own areas 36-40.",
            "next_step": None,
        }
    ...
    # Log to memory
    log_to_memory(f"JanusPike COMPLETED {task_id}: {task_title} — {result_message}")
    ...
    return {
        ...
        "message": f"I am JanusPike. I worked on {task_id}: {task_title}.\n  Result: {result_message}\n  Task marked DONE.",
        "next_step": None,
    }
