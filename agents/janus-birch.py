...
def do_work(task):
    ...
    log_to_memory(f"JANUS-JanusBirch security check for areas 11-15")
    return f"Security check complete for areas 11-15."
...
def run(message="work", context=None):
    ...
    if not my_tasks:
        return {
            "agent": "JanusBirch",
            "action": "idle",
            "message": f"I am JanusBirch. No tasks assigned to me on the board. I own areas 11-15.",
            "next_step": None,
        }
    ...
    # Log to memory
    log_to_memory(f"JanusBirch COMPLETED {task_id}: {task_title} — {result_message}")
    ...
    return {
        ...
        "message": f"I am JanusBirch. I worked on {task_id}: {task_title}.\n  Result: {result_message}\n  Task marked DONE.",
        "next_step": None,
    }
