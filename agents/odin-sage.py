...
def do_work(task):
    """Odin agent: research standards for areas 01-05."""
    log_to_memory(f"ODIN-OdinSage researching standards for areas 01-05")
    return f"Researched standards for areas 01-05. Knowledge base updated."
...
def run(message="work", context=None):
    ...
    if not my_tasks:
        return {
            "agent": "OdinSage",
            "action": "idle",
            "message": f"I am OdinSage. No tasks assigned to me on the board. I own areas 01-05.",
            "next_step": None,
        }
    ...
    # Log to memory
    log_to_memory(f"OdinSage COMPLETED {task_id}: {task_title} — {result_message}")
    ...
    return {
        ...
        "message": f"I am OdinSage. I worked on {task_id}: {task_title}.\n  Result: {result_message}\n  Task marked DONE.",
        "next_step": None,
    }
