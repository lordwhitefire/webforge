...
def do_work(task):
    ...
    log_to_memory(f"JrOcean working on: {title}")
...
def run(message="work", context=None):
    ...
    if not my_tasks:
        return {
            "agent": "JrOcean",
            "action": "idle",
            "message": f"I am JrOcean. No tasks assigned to me on the board. I own areas 71-75.",
            "next_step": None,
        }
    ...
    return {
        ...
        "message": f"I am JrOcean. I worked on {task_id}: {task_title}.\n  Result: {result_message}\n  Task marked DONE.",
        "next_step": None,
    }
