#!/usr/bin/env python3
"""
JrOcean — Build Department Worker

STANDALONE script. Does the actual work.

Role: I am JrOcean. I am a Junior Frontend Developer. I report to Aurora.
Areas: 26-30
"""
...
def log_to_memory(message: str, kind: str = "note"):
    try:
        ...
        session_append(message, agent="JrOcean", kind=kind)
    except Exception:
        pass
...
def ack_task(task: dict):
    ...
    try:
        ...
        if assigned_msg:
            from mailbox import Mailbox
            Mailbox("jr-ocean").ack(assigned_msg["id"], "On it — " + title)
    except Exception as e:
        _log(f"ack failed: {e}")
...
def run(message: str = "work", context: dict = None) -> dict:
    _log(f"JrOcean triggered (task_id={os.environ.get('WEBFORGE_TASK_ID', '')!r})")
    ...
    if explicit_id:
        ...
        if explicit:
            ...
            if task and (task.get("owner") or "").lower() == "jr-ocean":
                my_tasks = [task]

    if not my_tasks:
        no_work_msg = f"I am JrOcean. No tasks assigned to me."
        _log(no_work_msg)
    ...
    notify_agent("Developer", "TASK_DONE",
                 f"@JrOcean completed {task_id}: {task_title}. Result: {result_message}",
                 task_id)
    log_to_memory(f"JrOcean COMPLETED {task_id}: {task_title} — {result_message}",
                  kind="decision")
    ...
    return {
        ...
        "message": (
            f"I am JrOcean. I worked on {task_id}: {task_title}.\n"
...
if __name__ == "__main__":
    ...
    _log(f"=== JrOcean starting (pid={os.getpid()}) ===")
    r = run(msg)
    _log(f"=== JrOcean done: action={r.get('action')} ===")
