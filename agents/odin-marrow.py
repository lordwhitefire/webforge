#!/usr/bin/env python3
"""
OdinMarrow — Intelligence Department
STANDALONE script. Checks board for assigned tasks and works on them.

Role: I am Odin-Marrow. I am an Odin Agent in the Intelligence team. I report to Odin-Lead.
Areas: 61-65
"""
...
def do_work(task):
    """Odin agent: research standards for areas 61-65."""
    log_to_memory(f"ODIN-OdinMarrow researching standards for areas 61-65")
    return f"Researched standards for areas 61-65. Knowledge base updated."
...
def run(message="work", context=None):
    ...
    if not my_tasks:
        return {
            "agent": "OdinMarrow",
            "action": "idle",
            "message": f"I am OdinMarrow. No tasks assigned to me on the board. I own areas 61-65.",
            "next_step": None,
        }
    ...
    # Log to memory
    log_to_memory(f"OdinMarrow COMPLETED {task_id}: {task_title} — {result_message}")
    ...
    return {
        ...
        "message": f"I am OdinMarrow. I worked on {task_id}: {task_title}.\n  Result: {result_message}\n  Task marked DONE.",
        "next_step": None,
    }
