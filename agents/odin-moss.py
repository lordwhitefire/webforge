#!/usr/bin/env python3
"""
OdinMoss — Intelligence Department
STANDALONE script. Checks board for assigned tasks and works on them.

Role: I am Odin-Moss. I am an Odin Agent in the Intelligence team. I report to Odin-Lead.
Areas: 21-25
"""
...
def do_work(task):
    """Odin agent: research standards for areas 21-25."""
    log_to_memory(f"ODIN-OdinMoss researching standards for areas 21-25")
    return f"Researched standards for areas 21-25. Knowledge base updated."
...
def run(message="work", context=None):
    ...
    if not my_tasks:
        return {
            "agent": "OdinMoss",
            "action": "idle",
            "message": f"I am OdinMoss. No tasks assigned to me on the board. I own areas 21-25.",
            "next_step": None,
        }
    ...
    # Log to memory
    log_to_memory(f"OdinMoss COMPLETED {task_id}: {task_title} — {result_message}")
    ...
    return {
        ...
        "message": f"I am OdinMoss. I worked on {task_id}: {task_title}.\n  Result: {result_message}\n  Task marked DONE.",
        "next_step": None,
    }
