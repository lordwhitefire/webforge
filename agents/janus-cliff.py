...
def do_work(task):
    ...
    log_to_memory(f"JANUS-JanusCliff security check for areas 16-20")
    return f"Security check complete for areas 16-20."
...
def run(message="work", context=None):
    ...
    if not my_tasks:
        return {
            "agent": "JanusCliff",
            "action": "idle",
            "message": f"I am JanusCliff. No tasks assigned to me on the board. I own areas 16-20.",
            "next_step": None,
        }
    ...
    # Log to memory
    log_to_memory(f"JanusCliff COMPLETED {task_id}: {task_title} — {result_message}")
    ...
    return {
        ...
        "message": f"I am JanusCliff. I worked on {task_id}: {task_title}.\n  Result: {result_message}\n  Task marked DONE.",
        "next_step": None,
    }
