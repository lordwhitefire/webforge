#!/usr/bin/env python3
"""
SrEarth — Build Department
STANDALONE script. Checks board for assigned tasks and works on them.

Role: I am Sr-Earth. I am a Senior Developer in the DB/Infra sub-department. I report to Lead-Zen.
Areas: N/A
"""
...
def do_work(task):
    ...
    log_to_memory(f"SR-SrEarth reviewing: {title}")
...
def run(message="work", context=None):
    ...
    if not my_tasks:
        return {
            "agent": "SrEarth",
            "action": "idle",
            "message": f"I am SrEarth. No tasks assigned to me on the board. I own areas assigned.",
            "next_step": None,
        }
    ...
    # Log to memory
    log_to_memory(f"SrEarth COMPLETED {task_id}: {task_title} — {result_message}")
    ...
    return {
        ...
        "message": f"I am SrEarth. I worked on {task_id}: {task_title}.\n  Result: {result_message}\n  Task marked DONE.",
        "next_step": None,
    }
