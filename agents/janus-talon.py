...
def do_work(task):
    ...
    log_to_memory(f"JANUS-JanusTalon security check for areas 56-60")
    return f"Security check complete for areas 56-60."
...
def run(message="work", context=None):
    ...
    if not my_tasks:
        return {
            "agent": "JanusTalon",
            "action": "idle",
            "message": f"I am JanusTalon. No tasks assigned to me on the board. I own areas 56-60.",
            "next_step": None,
        }
    ...
    # Log to memory
    log_to_memory(f"JanusTalon COMPLETED {task_id}: {task_title} — {result_message}")
    ...
    return {
        ...
        "message": f"I am JanusTalon. I worked on {task_id}: {task_title}.\n  Result: {result_message}\n  Task marked DONE.",
        "next_step": None,
    }
