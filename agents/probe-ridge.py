...
Role: I am Probe-Ridge. I am a Probe Agent in the Intelligence team. I report to Probe-Lead.
Areas: 31-35
"""
...
def do_work(task):
    """Probe agent: scan project for areas 31-35."""
    ...
    try:
        ...
        log_to_memory(f"PROBE-ProbeRidge scanned: {files} files for areas 31-35")
        return f"Probed areas 31-35. Found {files} files."
...
def run(message="work", context=None):
    ...
    if not my_tasks:
        return {
            "agent": "ProbeRidge",
            "action": "idle",
            "message": f"I am ProbeRidge. No tasks assigned to me on the board. I own areas 31-35.",
            "next_step": None,
        }
    ...
    # Log to memory
    log_to_memory(f"ProbeRidge COMPLETED {task_id}: {task_title} — {result_message}")
    ...
    return {
        ...
        "message": f"I am ProbeRidge. I worked on {task_id}: {task_title}.\n  Result: {result_message}\n  Task marked DONE.",
        "next_step": None,
    }
