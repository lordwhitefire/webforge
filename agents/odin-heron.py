#!/usr/bin/env python3
"""
OdinHeron — Intelligence Department
STANDALONE script. Checks board for assigned tasks and works on them.

Role: I am Odin-Heron. I am an Odin Agent in the Intelligence team. I report to Odin-Lead.
Areas: 71-75
"""
...
def do_work(task):
    """Odin agent: research standards for areas 71-75."""
    log_to_memory(f"ODIN-OdinHeron researching standards for areas 71-75")
    return f"Researched standards for areas 71-75. Knowledge base updated."
...
def run(message="work", context=None):
    ...
    if not my_tasks:
        return {
            "agent": "OdinHeron",
            "action": "idle",
            "message": f"I am OdinHeron. No tasks assigned to me on the board. I own areas 71-75.",
            "next_step": None,
        }
    ...
    # Log to memory
    log_to_memory(f"OdinHeron COMPLETED {task_id}: {task_title} — {result_message}")
    ...
    return {
        ...
        "message": f"I am OdinHeron. I worked on {task_id}: {task_title}.\n  Result: {result_message}\n  Task marked DONE.",
        "next_step": None,
    }
