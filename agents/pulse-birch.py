#!/usr/bin/env python3
"""
PulseBirch — Quality Department
STANDALONE script. Checks board for assigned tasks and works on them.

Role: I am Pulse-Birch. I am a Bug Fixer Agent. I report to Pulse-Core.
Areas: 11-15
"""
...
def do_work(task):
    ...
    log_to_memory(f"PULSE-PulseBirch fixing: {title}")
...
def run(message="work", context=None):
    ...
    if not my_tasks:
        return {
            "agent": "PulseBirch",
            "action": "idle",
            "message": f"I am PulseBirch. No tasks assigned to me on the board. I own areas 11-15.",
            "next_step": None,
        }
    ...
    # Log to memory
    log_to_memory(f"PulseBirch COMPLETED {task_id}: {task_title} — {result_message}")
    ...
    return {
        ...
        "message": f"I am PulseBirch. I worked on {task_id}: {task_title}.\n  Result: {result_message}\n  Task marked DONE.",
        "next_step": None,
    }
