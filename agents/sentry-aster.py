...
def run(message="work", context=None):
    ...
    if not my_tasks:
        return {
            "agent": "SentryAster",
            "action": "idle",
            "message": f"I am SentryAster. No tasks assigned to me on the board. I own areas 81-82.",
            "next_step": None,
        }
    ...
    # Log to memory
    log_to_memory(f"SentryAster COMPLETED {task_id}: {task_title} — {result_message}")
    ...
    return {
        ...
        "message": f"I am SentryAster. I worked on {task_id}: {task_title}.\n  Result: {result_message}\n  Task marked DONE.",
        "next_step": None,
    }
