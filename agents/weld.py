...
def run(message="work", context=None):
    ...
    if not my_tasks:
        return {
            "agent": "Weld",
            "action": "idle",
            "message": f"I am Weld. No tasks assigned to me on the board. I own areas assigned.",
            "next_step": None,
        }
    ...
    # Log to memory
    log_to_memory(f"Weld COMPLETED {task_id}: {task_title} — {result_message}")
    ...
    return {
        ...
        "message": f"I am Weld. I worked on {task_id}: {task_title}.\n  Result: {result_message}\n  Task marked DONE.",
        "next_step": None,
    }
