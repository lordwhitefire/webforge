#!/usr/bin/env python3
"""
SentryMarrow — Quality Department
STANDALONE script. Checks board for assigned tasks and works on them.

Role: I am Sentry-Marrow. I am a I review test scripts written by Pixel agents agent. I report to Sentry-Core.
Areas: 61-65
"""
...
def run(message="work", context=None):
    ...
    if not my_tasks:
        return {
            "agent": "SentryMarrow",
            "action": "idle",
            "message": f"I am SentryMarrow. No tasks assigned to me on the board. I own areas 61-65.",
            "next_step": None,
        }
    ...
    # Log to memory
    log_to_memory(f"SentryMarrow COMPLETED {task_id}: {task_title} — {result_message}")
    ...
    return {
        ...
        "message": f"I am SentryMarrow. I worked on {task_id}: {task_title}.\n  Result: {result_message}\n  Task marked DONE.",
        "next_step": None,
    }
