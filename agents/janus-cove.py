...
def do_work(task):
    ...
    log_to_memory(f"JANUS-JanusCove security check for areas 46-50")
    return f"Security check complete for areas 46-50."
...
def run(message="work", context=None):
    ...
    if not my_tasks:
        return {
            "agent": "JanusCove",
            "action": "idle",
            "message": f"I am JanusCove. No tasks assigned to me on the board. I own areas 46-50.",
            "next_step": None,
        }
    ...
    # Log to memory
    log_to_memory(f"JanusCove COMPLETED {task_id}: {task_title} — {result_message}")
    ...
    return {
        ...
        "message": f"I am JanusCove. I worked on {task_id}: {task_title}.\n  Result: {result_message}\n  Task marked DONE.",
        "next_step": None,
    }
