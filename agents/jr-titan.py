#!/usr/bin/env python3
"""
JrTitan — Build Department Worker

STANDALONE script. Does the actual work.

Role: I am JrTitan. I am a Junior Backend Developer. I report to Sr-Earth.
Areas: varies
"""
...
def log_to_memory(message: str, kind: str = "note"):
    try:
        ...
        session_append(message, agent="JrTitan", kind=kind)
    except Exception:
        pass
...
def ack_task(task: dict):
    ...
    try:
        ...
        if assigned_msg:
            from mailbox import Mailbox
            Mailbox("jr-titan").ack(assigned_msg["id"], "On it — " + title)
    except Exception as e:
        _log(f"ack failed: {e}")
    ...
    notify_agent("Developer", "TASK_ACK",
                 "@JrTitan ACK'd " + task_id + ": Junior Backend Developer.", task_id)
    log_to_memory("JrTitan ACK — picked up " + task_id + ": Junior Backend Developer", kind="decision")
...
def run(message: str = "work", context: dict = None) -> dict:
    _log(f"JrTitan triggered (task_id={os.environ.get('WEBFORGE_TASK_ID', '')!r})")
    ...
    if explicit_id:
        ...
        if explicit:
            ...
            if task and (task.get("owner") or "").lower() == "jr-titan":
                my_tasks = [task]

    if not my_tasks:
        no_work_msg = f"I am JrTitan. No tasks assigned to me."
        _log(no_work_msg)
    ...
    notify_agent("Developer", "TASK_DONE",
                 f"@JrTitan completed {task_id}: {task_title}. Result: {result_message}",
                 task_id)
    log_to_memory(f"JrTitan COMPLETED {task_id}: {task_title} — {result_message}",
                  kind="decision")
    ...
    return {
        ...
        "message": (
            f"I am JrTitan. I worked on {task_id}: {task_title}.\n"
...
if __name__ == "__main__":
    ...
    _log(f"=== JrTitan starting (pid={os.getpid()}) ===")
    r = run(msg)
    _log(f"=== JrTitan done: action={r.get('action')} ===")
