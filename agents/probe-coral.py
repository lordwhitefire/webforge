#!/usr/bin/env python3
"""
ProbeCoral — Intelligence Department
STANDALONE script. Checks board for assigned tasks and works on them.

Role: I am Probe-Coral. I am a Probe Agent in the Intelligence team. I report to Probe-Lead.
Areas: 41-45
"""
...
def do_work(task):
    """Probe agent: scan project for areas 41-45."""
    ...
    try:
        ...
        log_to_memory(f"PROBE-ProbeCoral scanned: {files} files for areas 41-45")
        return f"Probed areas 41-45. Found {files} files."
...
def run(message="work", context=None):
    ...
    if not my_tasks:
        return {
            "agent": "ProbeCoral",
            "action": "idle",
            "message": f"I am ProbeCoral. No tasks assigned to me on the board. I own areas 41-45.",
            "next_step": None,
        }
    ...
    # Log to memory
    log_to_memory(f"ProbeCoral COMPLETED {task_id}: {task_title} — {result_message}")
    ...
    return {
        ...
        "message": f"I am ProbeCoral. I worked on {task_id}: {task_title}.\n  Result: {result_message}\n  Task marked DONE.",
        "next_step": None,
    }
