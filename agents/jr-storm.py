#!/usr/bin/env python3
"""
JrStorm — Build Department Worker

STANDALONE script. Does the actual work.

Role: I am Jr-Storm. I am a Junior Frontend Developer. I report to Sr-Quill2.
Areas: varies
"""
...
def log_to_memory(message: str, kind: str = "note"):
    try:
        ...
        session_append(message, agent="JrStorm", kind=kind)
    except Exception:
        pass
...
def ack_task(task: dict):
    ...
    try:
        ...
        if assigned_msg:
            from mailbox import Mailbox
            Mailbox("jr-storm").ack(assigned_msg["id"], "On it — " + title)
    except Exception as e:
        _log(f"ack failed: {e}")
    ...
    try:
        ...
        my_def = get_agent("jr-storm")
        superior = my_def.reports_to if my_def and my_def.reports_to else "Hephaestus"
    except ImportError:
        superior = "Hephaestus"
    notify_agent(superior, "TASK_ACK",
                 "ACK — picked up " + task_id + ": JrStorm.", task_id)
    notify_agent("Developer", "TASK_ACK",
                 "@JrStorm ACK'd " + task_id + ": JrStorm.", task_id)
    log_to_memory("JrStorm ACK — picked up " + task_id + ": JrStorm", kind="decision")
...
def run(message: str = "work", context: dict = None) -> dict:
    _log(f"JrStorm triggered (task_id={os.environ.get('WEBFORGE_TASK_ID', '')!r})")
    ...
    if explicit_id:
        ...
        if explicit:
            ...
            if task and (task.get("owner") or "").lower() == "jr-storm":
                my_tasks = [task]

    if not my_tasks:
        no_work_msg = f"I am JrStorm. No tasks assigned to me."
        _log(no_work_msg)
    ...
    notify_agent("Developer", "TASK_DONE",
                 f"@JrStorm completed {task_id}: {task_title}. Result: {result_message}",
                 task_id)
    log_to_memory(f"JrStorm COMPLETED {task_id}: {task_title} — {result_message}",
                  kind="decision")
    ...
    return {
        ...
        "message": (
            f"I am JrStorm. I worked on {task_id}: {task_title}.\n"
...
if __name__ == "__main__":
    ...
    _log(f"=== JrStorm starting (pid={os.getpid()}) ===")
    r = run(msg)
    _log(f"=== JrStorm done: action={r.get('action')} ===")
