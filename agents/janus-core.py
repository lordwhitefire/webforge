...
def do_work(task):
    ...
    log_to_memory(f"JANUS-JanusCore security check for areas ")
    return f"Security check complete for areas ."
...
def run(message="work", context=None):
    ...
    if not my_tasks:
        return {
            "agent": "JanusCore",
            "action": "idle",
            "message": f"I am JanusCore. No tasks assigned to me on the board. I own areas assigned.",
            "next_step": None,
        }
    ...
    # Log to memory
    log_to_memory(f"JanusCore COMPLETED {task_id}: {task_title} — {result_message}")
    ...
    return {
        ...
        "message": f"I am JanusCore. I worked on {task_id}: {task_title}.\n  Result: {result_message}\n  Task marked DONE.",
        "next_step": None,
    }
