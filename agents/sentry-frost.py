#!/usr/bin/env python3
"""
SentryFrost — Quality Department
STANDALONE script. Checks board for assigned tasks and works on them.

Role: I am Sentry-Frost. I am a I review test scripts written by Pixel agents agent. I report to Sentry-Core.
Areas: 76-80
"""
...
def run(message="work", context=None):
    ...
    if not my_tasks:
        return {
            "agent": "SentryFrost",
            "action": "idle",
            "message": f"I am SentryFrost. No tasks assigned to me on the board. I own areas 76-80.",
            "next_step": None,
        }
    ...
    # Log to memory
    log_to_memory(f"SentryFrost COMPLETED {task_id}: {task_title} — {result_message}")
    ...
    return {
        ...
        "message": f"I am SentryFrost. I worked on {task_id}: {task_title}.\n  Result: {result_message}\n  Task marked DONE.",
        "next_step": None,
    }
