#!/usr/bin/env python3
"""
JrCove — Build Department Worker

STANDALONE script. Does the actual work.

Role: I am JrCove. I am a Junior Frontend Developer. I report to Sr-Hale.
Areas: 66-70
"""
...
def log_to_memory(message: str, kind: str = "note"):
    try:
        ...
        session_append(message, agent="JrCove", kind=kind)
    except Exception:
        pass
...
def ack_task(task: dict):
    ...
    try:
        ...
        if assigned_msg:
            from mailbox import Mailbox
            Mailbox("jr-cove").ack(assigned_msg["id"], "On it — " + title)
    except Exception as e:
        _log(f"ack failed: {e}")
    ...
    notify_agent("Developer", "TASK_ACK",
                 "@JrCove ACK'd " + task_id + ": Junior Frontend Developer.", task_id)
    log_to_memory("JrCove ACK — picked up " + task_id + ": Junior Frontend Developer", kind="decision")
...
def run(message: str = "work", context: dict = None) -> dict:
    _log(f"JrCove triggered (task_id={os.environ.get('WEBFORGE_TASK_ID', '')!r})")
    ...
    if explicit_id:
        ...
        if explicit:
            ...
            if task and (task.get("owner") or "").lower() == "jr-cove":
                my_tasks = [task]

    if not my_tasks:
        no_work_msg = f"I am JrCove. No tasks assigned to me."
        _log(no_work_msg)
    ...
    notify_agent("Developer", "TASK_DONE",
                 f"@JrCove completed {task_id}: {task_title}. Result: {result_message}",
                 task_id)
    log_to_memory(f"JrCove COMPLETED {task_id}: {task_title} — {result_message}",
                  kind="decision")
    ...
    return {
        ...
        "message": (
            f"I am JrCove. I worked on {task_id}: {task_title}.\n"
...
if __name__ == "__main__":
    ...
    _log(f"=== JrCove starting (pid={os.getpid()}) ===")
    r = run(msg)
    _log(f"=== JrCove done: action={r.get('action')} ===")
