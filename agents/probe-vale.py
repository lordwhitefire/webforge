#!/usr/bin/env python3
"""
ProbeVale — Intelligence Department
STANDALONE script. Checks board for assigned tasks and works on them.

Role: I am Probe-Vale. I am a Probe Agent in the Intelligence team. I report to Probe-Lead.
Areas: 46-50
"""
...
def do_work(task):
    """Probe agent: scan project for areas 46-50."""
    ...
    try:
        ...
        log_to_memory(f"PROBE-ProbeVale scanned: {files} files for areas 46-50")
        return f"Probed areas 46-50. Found {files} files."
...
def run(message="work", context=None):
    ...
    if not my_tasks:
        return {
            "agent": "ProbeVale",
            "action": "idle",
            "message": f"I am ProbeVale. No tasks assigned to me on the board. I own areas 46-50.",
            "next_step": None,
        }
    ...
    # Log to memory
    log_to_memory(f"ProbeVale COMPLETED {task_id}: {task_title} — {result_message}")
    ...
    return {
        ...
        "message": f"I am ProbeVale. I worked on {task_id}: {task_title}.\n  Result: {result_message}\n  Task marked DONE.",
        "next_step": None,
    }
