#!/usr/bin/env python3
"""
OdinReed — Intelligence Department
STANDALONE script. Checks board for assigned tasks and works on them.

Role: I am Odin-Reed. I am an Odin Agent in the Intelligence team. I report to Odin-Lead.
Areas: 06-10
"""
...
def do_work(task):
    """Odin agent: research standards for areas 06-10."""
    log_to_memory(f"ODIN-OdinReed researching standards for areas 06-10")
    return f"Researched standards for areas 06-10. Knowledge base updated."
...
def run(message="work", context=None):
    ...
    if not my_tasks:
        return {
            "agent": "OdinReed",
            "action": "idle",
            "message": f"I am OdinReed. No tasks assigned to me on the board. I own areas 06-10.",
            "next_step": None,
        }
    ...
    # Log to memory
    log_to_memory(f"OdinReed COMPLETED {task_id}: {task_title} — {result_message}")
    ...
    return {
        ...
        "message": f"I am OdinReed. I worked on {task_id}: {task_title}.\n  Result: {result_message}\n  Task marked DONE.",
        "next_step": None,
    }
