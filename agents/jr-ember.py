#!/usr/bin/env python3
"""
JrEmber — Build Department Worker

STANDALONE script. Does the actual work.

Role: I am JrEmber. I am a Junior Database/Infra Developer. I report to Sr-Fire.
Areas: 46-50
"""
...
def log_to_memory(message: str, kind: str = "note"):
    try:
        ...
        session_append(message, agent="JrEmber", kind=kind)
    except Exception:
        pass
...
def ack_task(task: dict):
    ...
    try:
        ...
        if assigned_msg:
            from mailbox import Mailbox
            Mailbox("jr-ember").ack(assigned_msg["id"], "On it — " + title)
    except Exception as e:
        _log(f"ack failed: {e}")
    ...
    notify_agent("Developer", "TASK_ACK",
                 "@JrEmber ACK'd " + task_id + ": Junior Database/Infra Developer.", task_id)
    log_to_memory("JrEmber ACK — picked up " + task_id + ": Junior Database/Infra Developer", kind="decision")
...
def run(message: str = "work", context: dict = None) -> dict:
    _log(f"JrEmber triggered (task_id={os.environ.get('WEBFORGE_TASK_ID', '')!r})")
    ...
    if explicit_id:
        ...
        if explicit:
            ...
            if task and (task.get("owner") or "").lower() == "jr-ember":
                my_tasks = [task]

    if not my_tasks:
        no_work_msg = f"I am JrEmber. No tasks assigned to me."
        _log(no_work_msg)
    ...
    notify_agent("Developer", "TASK_DONE",
                 f"@JrEmber completed {task_id}: {task_title}. Result: {result_message}",
                 task_id)
    log_to_memory(f"JrEmber COMPLETED {task_id}: {task_title} — {result_message}",
                  kind="decision")
    ...
    return {
        ...
        "message": (
            f"I am JrEmber. I worked on {task_id}: {task_title}.\n"
...
if __name__ == "__main__":
    ...
    _log(f"=== JrEmber starting (pid={os.getpid()}) ===")
    r = run(msg)
    _log(f"=== JrEmber done: action={r.get('action')} ===")
