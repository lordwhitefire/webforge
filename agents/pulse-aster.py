...
def do_work(task):
    ...
    log_to_memory(f"PULSE-PulseAster fixing: {title}")
...
def run(message="work", context=None):
    ...
    if not my_tasks:
        return {
            "agent": "PulseAster",
            "action": "idle",
            "message": f"I am PulseAster. No tasks assigned to me on the board. I own areas 81-82.",
            "next_step": None,
        }
    ...
    # Log to memory
    log_to_memory(f"PulseAster COMPLETED {task_id}: {task_title} — {result_message}")
    ...
    return {
        ...
        "message": f"I am PulseAster. I worked on {task_id}: {task_title}.\n  Result: {result_message}\n  Task marked DONE.",
        "next_step": None,
    }
