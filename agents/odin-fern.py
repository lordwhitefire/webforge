...
def do_work(task):
    """Odin agent: research standards for areas 31-35."""
    log_to_memory(f"ODIN-OdinFern researching standards for areas 31-35")
    return f"Researched standards for areas 31-35. Knowledge base updated."
...
def run(message="work", context=None):
    ...
    if not my_tasks:
        return {
            "agent": "OdinFern",
            "action": "idle",
            "message": f"I am OdinFern. No tasks assigned to me on the board. I own areas 31-35.",
            "next_step": None,
        }
    ...
    # Log to memory
    log_to_memory(f"OdinFern COMPLETED {task_id}: {task_title} — {result_message}")
    ...
    return {
        ...
        "message": f"I am OdinFern. I worked on {task_id}: {task_title}.\n  Result: {result_message}\n  Task marked DONE.",
        "next_step": None,
    }
