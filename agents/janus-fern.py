#!/usr/bin/env python3
"""
JanusFern — Quality Department
STANDALONE script. Checks board for assigned tasks and works on them.

Role: I am Janus-Fern. I am a Security & Compliance Agent. I report to Janus-Core.
Areas: 31-35
"""
...
def do_work(task):
    ...
    log_to_memory(f"JANUS-JanusFern security check for areas 31-35")
    return f"Security check complete for areas 31-35."
...
def run(message="work", context=None):
    ...
    if not my_tasks:
        return {
            "agent": "JanusFern",
            "action": "idle",
            "message": f"I am JanusFern. No tasks assigned to me on the board. I own areas 31-35.",
            "next_step": None,
        }
    ...
    # Log to memory
    log_to_memory(f"JanusFern COMPLETED {task_id}: {task_title} — {result_message}")
    ...
    return {
        ...
        "message": f"I am JanusFern. I worked on {task_id}: {task_title}.\n  Result: {result_message}\n  Task marked DONE.",
        "next_step": None,
    }
