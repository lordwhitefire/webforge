#!/usr/bin/env python3
"""
JrNickel — Build Department Worker

STANDALONE script. Does the actual work.

Role: I am JrNickel. I am a Junior Database/Infra Developer. I report to Sr-Water.
Areas: 21-25
"""
...
CONSTRAINTS = {
    "role": "Junior Database/Infra Developer",
...
def log_to_memory(message: str, kind: str = "note"):
    try:
        ...
        session_append(message, agent="JrNickel", kind=kind)
    except Exception:
        pass
...
def ack_task(task: dict):
    ...
    try:
        ...
        if assigned_msg:
            from mailbox import Mailbox
            Mailbox("jr-nickel").ack(assigned_msg["id"], "On it — " + title)
    except Exception as e:
        _log(f"ack failed: {e}")
    ...
    try:
        ...
        my_def = get_agent("jr-nickel")
        superior = my_def.reports_to if my_def and my_def.reports_to else "Hephaestus"
    except ImportError:
        superior = "Hephaestus"
    notify_agent(superior, "TASK_ACK",
                 "ACK — picked up " + task_id + ": Junior Database/Infra Developer.", task_id)
    notify_agent("Developer", "TASK_ACK",
                 "@JrNickel ACK'd " + task_id + ": Junior Database/Infra Developer.", task_id)
    log_to_memory("JrNickel ACK — picked up " + task_id + ": Junior Database/Infra Developer", kind="decision")
...
def do_default_work(task: dict) -> str:
    ...
    title_str = "Junior Database/Infra Developer"
    owner_str = "JrNickel"
    areas_str = "21-25"
...
def run(message: str = "work", context: dict = None) -> dict:
    _log(f"JrNickel triggered (task_id={os.environ.get('WEBFORGE_TASK_ID', '')!r})")
    ...
    if explicit_id:
        ...
        if explicit:
            ...
            if task and (task.get("owner") or "").lower() == "jr-nickel":
                my_tasks = [task]

    if not my_tasks:
        no_work_msg = f"I am JrNickel. No tasks assigned to me."
        _log(no_work_msg)
    ...
    notify_agent("Developer", "TASK_DONE",
                 f"@JrNickel completed {task_id}: {task_title}. Result: {result_message}",
                 task_id)
    log_to_memory(f"JrNickel COMPLETED {task_id}: {task_title} — {result_message}",
                  kind="decision")
    ...
    return {
        ...
        "message": (
            f"I am JrNickel. I worked on {task_id}: {task_title}.\n"
...
if __name__ == "__main__":
    ...
    _log(f"=== JrNickel starting (pid={os.getpid()}) ===")
    r = run(msg)
    _log(f"=== JrNickel done: action={r.get('action')} ===")
