...
def run(message="work", context=None):
    ...
    if not my_tasks:
        return {
            "agent": "SentryWisp",
            "action": "idle",
            "message": f"I am SentryWisp. No tasks assigned to me on the board. I own areas 41-45.",
            "next_step": None,
        }
    ...
    # Log to memory
    log_to_memory(f"SentryWisp COMPLETED {task_id}: {task_title} — {result_message}")
    ...
    return {
        ...
        "message": f"I am SentryWisp. I worked on {task_id}: {task_title}.\n  Result: {result_message}\n  Task marked DONE.",
        "next_step": None,
    }
