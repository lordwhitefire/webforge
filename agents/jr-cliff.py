...
def ack_task(task: dict):
    ...
    try:
        ...
        my_def = get_agent("jr-cliff")
        superior = my_def.reports_to if my_def and my_def.reports_to else "Hephaestus"
    except ImportError:
        superior = "Hephaestus"
    notify_agent(superior, "TASK_ACK",
                 "ACK — picked up " + task_id + ": JrCliff.", task_id)
    notify_agent("Developer", "TASK_ACK",
                 "@JrCliff ACK'd " + task_id + ": JrCliff.", task_id)
    log_to_memory("JrCliff ACK — picked up " + task_id + ": JrCliff", kind="decision")
