#!/usr/bin/env python3
"""
JanusMarrow — Quality Department
STANDALONE script. Checks board for assigned tasks and works on them.

Role: I am Janus-Marrow. I am a Security & Compliance Agent. I report to Janus-Core.
Areas: 61-65
"""
...
def do_work(task):
    ...
    log_to_memory(f"JANUS-JanusMarrow security check for areas 61-65")
    return f"Security check complete for areas 61-65."
...
def run(message="work", context=None):
    ...
    if not my_tasks:
        return {
            "agent": "JanusMarrow",
            "action": "idle",
            "message": f"I am JanusMarrow. No tasks assigned to me on the board. I own areas 61-65.",
            "next_step": None,
        }
    ...
    # Log to memory
    log_to_memory(f"JanusMarrow COMPLETED {task_id}: {task_title} — {result_message}")
    ...
    return {
        ...
        "message": f"I am JanusMarrow. I worked on {task_id}: {task_title}.\n  Result: {result_message}\n  Task marked DONE.",
        "next_step": None,
    }
