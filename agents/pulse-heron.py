...
def do_work(task):
    ...
    log_to_memory(f"PULSE-PulseHeron fixing: {title}")
...
def run(message="work", context=None):
    ...
    if not my_tasks:
        return {
            "agent": "PulseHeron",
            "action": "idle",
            "message": f"I am PulseHeron. No tasks assigned to me on the board. I own areas 71-75.",
            "next_step": None,
        }
    ...
    # Log to memory
    log_to_memory(f"PulseHeron COMPLETED {task_id}: {task_title} — {result_message}")
    ...
    return {
        ...
        "message": f"I am PulseHeron. I worked on {task_id}: {task_title}.\n  Result: {result_message}\n  Task marked DONE.",
        "next_step": None,
    }
