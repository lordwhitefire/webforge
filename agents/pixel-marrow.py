...
def do_work(task):
    ...
    try:
        ...
        log_to_memory(f"PIXEL-PixelMarrow ran tests for areas 61-65")
        return f"Tests run for areas 61-65."
    except:
        return f"Test run failed for areas 61-65."
...
def run(message="work", context=None):
    ...
    if not my_tasks:
        return {
            "agent": "PixelMarrow",
            "action": "idle",
            "message": f"I am PixelMarrow. No tasks assigned to me on the board. I own areas 61-65.",
            "next_step": None,
        }
    ...
    # Log to memory
    log_to_memory(f"PixelMarrow COMPLETED {task_id}: {task_title} — {result_message}")
    ...
    return {
        ...
        "message": f"I am PixelMarrow. I worked on {task_id}: {task_title}.\n  Result: {result_message}\n  Task marked DONE.",
        "next_step": None,
    }
