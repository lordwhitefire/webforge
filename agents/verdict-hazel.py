...
def do_work(task):
    ...
    log_to_memory(f"VERDICT-VerdictHazel checking standards for areas 06-10")
    return f"Standards check complete for areas 06-10."
...
def run(message="work", context=None):
    ...
    if not my_tasks:
        return {
            "agent": "VerdictHazel",
            "action": "idle",
            "message": f"I am VerdictHazel. No tasks assigned to me on the board. I own areas 06-10.",
            "next_step": None,
        }
    ...
    # Log to memory
    log_to_memory(f"VerdictHazel COMPLETED {task_id}: {task_title} — {result_message}")
    ...
    return {
        ...
        "message": f"I am VerdictHazel. I worked on {task_id}: {task_title}.\n  Result: {result_message}\n  Task marked DONE.",
        "next_step": None,
    }
